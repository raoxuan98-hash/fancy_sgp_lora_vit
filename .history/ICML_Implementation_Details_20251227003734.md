...
1. **LDA (Linear Discriminant Analysis)**:
   ...
   - Final classifier encapsulated as `nn.Linear` layer for efficient inference

2. **QDA (Quadratic Discriminant Analysis)**:
   - Regularization α₁: 0.2 (for class-specific covariance)
   - Regularization α₂: 2.0 (for global covariance contribution)
   - Regularization α₃: 0.5 (for identity/spherical regularization)
   - Uses Cholesky decomposition with SVD fallback for matrix inversion
   - Employs batch processing for memory efficiency with large numbers of classes

3. **LR-RGDA (Low-Rank Regularized Gaussian Discriminant Analysis)**:
   - Rank parameter controls dimensionality reduction (default: 64)
   - Uses Woodbury matrix identity for efficient computation
   - Implements pre-computation strategy for affine and quadratic components
   - Supports batched SVD decomposition with configurable batch size
   - Separates discriminant function into linear and quadratic terms for optimization

### SGD Classifier

- Base training steps: 4000, with additional $3C$ steps where $C$ is the number of classes
- Learning rate: 10× base learning rate (initial: $10^{-3}$, final: $10^{-4}$)
- Momentum: 0.9
- Optimizer: AdamW with cosine annealing scheduler
- Samples per step: 128 from all classes
- Loss function: Cross-entropy loss
- Architecture choice: Linear classifiers for ViT/B-MocoV3, ViT/B-CLIP, and ViT/B-DINO; nonlinear classifiers for ViT/B-Sup21K
- Trained on compensated features with Cholesky decomposition for covariance sampling

### Implementation Details of Classifier Reconstruction

Here, we detail the implementation pipelines for all classifiers:

1. **SGD Classifier Implementation**:
   - Base training steps: 4000, with total steps of $4000 + 3C$ where $C$ is the number of classes
   - For example, in a Gaussian distribution set with 500 classes, total training steps are 5500
   - Pre-processing: Cholesky decomposition on covariance matrices for random sample generation
   - Training procedure: 128 samples drawn from all classes per step
   - Optimization: AdamW optimizer with initial learning rate of $10^{-3}$ and final rate of $10^{-4}$
   - Scheduling: Cosine annealing scheduler for learning rate decay
   - Performance observation: Classifier performance is insensitive to the number of samples per step
   - Architecture-specific findings:
     * Nonlinear classifiers outperform linear classifiers on ViT/B-Sup21K
     * Linear classifiers significantly outperform nonlinear classifiers on ViT/B-MocoV3, ViT/B-CLIP, and ViT/B-DINO
   - Final implementation: Linear classifiers for all SGD optimization-based classifiers

2. **LDA Classifier Implementation**:
   - Covariance matrix inversion: Computed using PyTorch's `torch.cholesky_inverse`
   - Affine transformation: Parameters encapsulated as `nn.Linear` layers
   - Regularization: Spherical regularization with α=0.1 applied to average covariance matrix
   - Memory efficiency: Streaming computation of global covariance to avoid stacking all covariance matrices

3. **RGDA Classifier Implementation**:
   - Matrix inversion: Each class's covariance matrix inverted using `torch.cholesky_inverse`
   - Inference optimization: Quadratic forms computed using `torch.einsum` for parallel computing
   - Numerical stability: Symmetrization of covariance matrices before decomposition
   - Fallback strategy: SVD decomposition used when Cholesky decomposition fails

4. **LR-RGDA Classifier Implementation**:
   - Matrix decomposition:
     * Average covariance matrix $\boldsymbol{\Sigma}_{\rm avg}$ and $\mathbf{M}_c$ matrices inverted using `torch.cholesky_inverse`
     * Individual class covariance matrices decomposed using `torch.svd_lowrank` with specified rank
   - Pre-computation strategy: Affine parameters and projection matrices computed in advance
   - Woodbury identity: Applied for efficient computation of the discriminant function
   - Batch processing: Configurable batch size for memory-efficient initialization
   - Component separation: Discriminant function split into linear and quadratic terms for optimization
