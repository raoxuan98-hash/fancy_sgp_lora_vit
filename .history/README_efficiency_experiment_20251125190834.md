...
### 自定义分类器参数
可以通过修改`exp_efficiency_comparison.py`中的`measure_build_time`函数来调整分类器参数：

```python
# 例如，修改QDA的正则化参数
builder = QDAClassifierBuilder(
    qda_reg_alpha1=0.1,  # 修改这里
    qda_reg_alpha2=0.1,  # 修改这里
    qda_reg_alpha3=0.1,  # 修改这里
    low_rank=False,
    device=device
)
```
...
```bash
# 创建批量运行脚本
for model in vit-b-p16 vit-b-p16-clip; do
    python exp_efficiency_comparison.py --model $model --output_dir "结果/${model}"
done
```
...
```bash
# GPU 0
CUDA_VISIBLE_DEVICES=0 python exp_efficiency_comparison.py --class_counts 50 100 &

# GPU 1  
CUDA_VISIBLE_DEVICES=1 python exp_efficiency_comparison.py --class_counts 200 500 &
```
...
```
.
├── exp_efficiency_comparison.py      # 主实验脚本
├── test_efficiency_experiment.py     # 测试脚本
├── design_efficiency_comparison.md   # 实验设计文档
├── README_efficiency_experiment.md   # 本文档
└── 实验结果保存/
    └── 效率对比实验/
        ├── efficiency_results_*.npz
        ├── efficiency_results_*.csv
