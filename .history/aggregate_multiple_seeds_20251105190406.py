...
def extract_results_from_log(log_path):
    ...
    try:
        ...
        matches = list(re.finditer(last_task_pattern, content))
        ...
        task_id = int(last_match.group(1))
        
        # 从最后一个匹配开始查找最终任务准确度部分
        remaining_content = content[last_match.start():]
        
        # 查找最终任务准确度部分
        final_task_pattern = r"   ── Final Task Accuracy (%) ──"
        final_task_match = re.search(final_task_pattern, remaining_content)
        ...
        # 提取所有变体的准确度
        accuracies = {}
        lines = remaining_content[final_task_match.start():].split('\n')
        ...
        avg_task_match = re.search(avg_task_pattern, remaining_content[final_task_match.start():])
        
        avg_accuracies = {}
        if avg_task_match:
            lines = remaining_content[avg_task_match.start():].split('\n')
            for line in lines:
                if "SeqFT +" in line and ":" in line:
                    parts = line.split(":")
                    variant = parts[0].strip()
        ...
        per_task_match = re.search(per_task_pattern, remaining_content[final_task_match.start():])
        
        per_task_trends = {}
        if per_task_match:
            lines = remaining_content[per_task_match.start():].split('\n')
            for line in lines:
                if "SeqFT +" in line and "→" in line:
                    parts = line.split(":")
                    variant = parts[0].strip()
