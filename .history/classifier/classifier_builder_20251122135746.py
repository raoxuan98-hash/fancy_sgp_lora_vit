...
class ClassifierReconstructor:
    ...
    def build_classifiers(self, variants: Dict[str, Dict[int, object]], classifier_type: Union[str, List[str]] = ["qda"]) -> Dict[str, Dict[str, nn.Module]]:
            ...
            if self.cached_Z is None:
                for name, stats in variants.items():
                    if stats:  # 找到第一个非空的variant
                        ...
                        break
            return SGDClassifierBuilder(cached_Z=self.cached_Z, device=self.device, epochs=3, lr=0.001)
