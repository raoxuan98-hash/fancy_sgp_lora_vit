...
class LowRankGaussianDA(nn.Module):
    def __init__(
        ...
        # 清理
        del U_eff, A_inv, global_cov, means, W_proj_tensor, batch_tensor
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # === 3. Quadratic Correction (Modified) ===
        # Q = 0.5 * u_c^T @ M^{-1} @ u_c
        
        # 1. 调整维度以利用广播矩阵乘法
        # u_c: [B, C, r] -> [B, C, r, 1] (变成列向量)
        # M_invs: [C, r, r] (自动广播为 [1, C, r, r] 以匹配 Batch)
        # Result: [B, C, r, 1]
        v_expanded = torch.matmul(self.M_invs, u_c.unsqueeze(-1))
        
        # 2. 去掉多余的维度
        # v: [B, C, r]
        v = v_expanded.squeeze(-1)
