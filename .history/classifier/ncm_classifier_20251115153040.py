...
class NCMClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 计算每个样本到每个类别均值的平方欧氏距离
        # (x - μ_c)^2 = x^2 + μ_c^2 - 2*x*μ_c
        
        # x_squared: [B, 1]
        x_squared = torch.sum(x ** 2, dim=1, keepdim=True)
        
        # mu_squared: [1, C]
        mu_squared = torch.sum(self.means ** 2, dim=1, keepdim=True).T
        
        # cross_term: [B, C]
        cross_term = 2 * torch.mm(x, self.means.T)
        
        # distances: [B, C]
        distances = x_squared + mu_squared - cross_term
        
        # NCM的判别函数: g_c(x) = -||x - μ_c||^2 + log π_c
        logits = -distances + self.log_priors.unsqueeze(0)
