...
class DistributionCompensator:
    ...
    def __init__(
        ...
        compensator_types: list[str] | None = None
    ):
        self.device = device
        ...
        if compensator_types is None:
            # 默认使用所有补偿器类型
            self.compensator_types = ["SeqFT", "SeqFT + linear_transform", "SeqFT + weaknonlinear_transform", "SeqFT + attention_transform"]
        else:
    ...
    def _update_variants_with_transforms(
        ...
        # 只计算需要的变换
        transforms = {}
        if "SeqFT + linear_transform" in self.compensator_types:
            transforms["linear_transform"] = self._compute_linear_transform(combined_before, combined_after)
        if "SeqFT + weaknonlinear_transform" in self.compensator_types:
            transforms["weaknonlinear_transform"] = self._compute_weaknonlinear_transform(combined_before, combined_after)
        if "SeqFT + attention_transform" in self.compensator_types:
            transforms["attention_transform"] = self._compute_attention_transform(combined_before, combined_after)
