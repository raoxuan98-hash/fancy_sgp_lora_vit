...
class DistributionCompensator:
    ...
    def _extract_combined_features(
        ...
        combined_cosine_sim = torch.nn.functional.cosine_similarity(combined_before, combined_after, dim=1).mean()
        combined_norm_diff = (combined_before.norm(p=2, dim=1) - combined_after.norm(p=2, dim=1)).abs().mean()
        
        logging.info(f"[Feature Analysis] Combined features - Cosine similarity: {combined_cosine_sim:.4f}, Norm difference: {combined_norm_diff:.4f}")
    ...
    def build_all_variants(
        ...
        # 一次性提取所有需要的特征，避免重复计算
        (current_before, current_after, aux_before, aux_after,
