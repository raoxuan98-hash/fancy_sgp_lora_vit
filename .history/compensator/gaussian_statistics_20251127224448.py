...
class LowRankGaussianStatistics:
    def __init__(
        ...
        # Perform low-rank SVD: cov ≈ U @ S @ V^T
        # For symmetric cov, U ≈ V, so we can use U @ sqrt(S) as L
        U, S, Vh = torch.svd_lowrank(cov, q=q, niter=4, M=None)
        U = U[:, :self.rank]
        S = S[:self.rank]
        S = torch.clamp(S, min=0.0)
        self.L = U * torch.sqrt(S).unsqueeze(0)  # (d, rank)
