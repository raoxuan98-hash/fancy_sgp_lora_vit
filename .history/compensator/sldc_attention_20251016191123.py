...
class SemanticDriftCompensator(BaseCompensator):
    """基于样本间距的语义漂移补偿器 (SDC)"""
    ...
    @torch.no_grad()
    def compensate(self, stats_dict, temperature=0.1):
        ...
        for cid, stat in stats_dict.items():
            ...
            cov = stat.covariance.to(self.device)
            similarities = torch.matmul(fb, mu.unsqueeze(1)).squeeze(1)
