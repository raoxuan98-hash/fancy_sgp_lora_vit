...
class AttentionCompensator(BaseCompensator):
    ...
    @torch.no_grad()
    def compensate(self, stats_dict, base_temperature=0.05, top_k=0, n_samples=2000, chunk_size=512):
        ...
        for cid, stat in stats_dict.items():
            ...
            # --- 均值补偿：使用点积相似度 ---
            # 直接计算点积: [N] = fb @ mu  (mu is [d])
            similarities = torch.matmul(fb, mu)  # shape: [N]
            ...
            for i in range(0, n_samples, chunk_size):
                ...
                # 使用点积相似度：samples @ fb^T
                sim = torch.matmul(samples, fb.t())  # [chunk, N]
