...
class AttentionCompensator(BaseCompensator):
    ...
    def compensate(
        ...
        for cid, stat in stats_dict.items():
            ...
            # ====== Logging: top/bottom 100 similarities ======
            if logger.isEnabledFor(logging.INFO):
                k_log = min(100, N)
                top_sim, _ = torch.topk(similarities, k_log, largest=True)
                bottom_sim, _ = torch.topk(similarities, k_log, largest=False)
                avg_top_sim = top_sim.mean().item()
                avg_bottom_sim = bottom_sim.mean().item()
                logger.info(
                    f"Class {cid} - Avg top-{k_log} similarity: {avg_top_sim:.4f}, "
                    f"Avg bottom-{k_log} similarity: {avg_bottom_sim:.4f}"
                )
            # ==================================================
