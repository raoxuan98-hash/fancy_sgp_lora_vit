        ...
        for class_id in range(num_classes):
            ...
            if test_count >= self.max_samples_per_class:
                ...
                if train_count >= needed_samples:
                    ...
                    # 记录移动的样本数
                    sampling_info['samples_moved_from_train_to_test'][int(class_id)] = int(needed_samples)
                    ...
                    # 为了避免训练集为空，将部分测试样本移回训练集
                    # 确保至少有一些训练样本用于学习
                    if total_count > 0:
                        # 将总样本的20%保留用于训练，至少1个样本
                        train_reserve = max(1, total_count // 5)
                        test_reserve = total_count - train_reserve
                        
                        # 重新分配样本
                        balanced_train_data.extend(train_data[train_indices][:train_reserve])
                        balanced_train_targets.extend([class_id] * train_reserve)
                        
                        # 测试集包含剩余样本
                        balanced_test_data.clear()
                        balanced_test_targets.clear()
                        balanced_test_data.extend(test_data[test_indices])
                        balanced_test_data.extend(train_data[train_indices][train_reserve:])
                        balanced_test_targets.extend([class_id] * test_reserve)
