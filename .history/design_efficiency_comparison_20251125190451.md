...
## 实验目标
比较以下分类器在不同类别数量下的计算效率：
1. Full-rank QDA
2. Low-rank QDA (rank-1, rank-8, rank-16, rank-32)
...
- **Low-rank QDA**:
  - qda_reg_alpha1=0.2, qda_reg_alpha2=0.2, qda_reg_alpha3=0.2
  - low_rank=True, rank=[1, 8, 16, 32] (分别测试)
...
- **Low-rank QDA**: O(C*D*r) 构建复杂度，O(C*r) 推理复杂度 (r∈{1,8,16,32})
- **SGD-based**: O(steps*B*D) 训练复杂度，O(D) 推理复杂度
- **LDA**: O(D²) 构建复杂度， O(D) 推理复杂度

其中C为类别数，D为特征维度(768)，r为秩(1,8,16,32)，B为批次大小，steps为训练步数(5000)

### 预期结论
1. 构建时间：Full-rank QDA > Low-rank QDA (rank-32) > Low-rank QDA (rank-16) > Low-rank QDA (rank-8) > Low-rank QDA (rank-1) > SGD-based > LDA
2. 推理时间：Full-rank QDA > Low-rank QDA (rank-32) > Low-rank QDA (rank-16) > Low-rank QDA (rank-8) > Low-rank QDA (rank-1) > LDA ≈ SGD-based
3. 内存使用：Full-rank QDA > Low-rank QDA (rank-32) > Low-rank QDA (rank-16) > Low-rank QDA (rank-8) > Low-rank QDA (rank-1) > SGD-based > LDA
4. 随着类别数量增加，Full-rank QDA的构建时间和内存使用将呈二次增长
5. Low-rank QDA的效率随rank值降低而提高，但可能牺牲一定的分类精度
...
def measure_build_time(
    ...
    **kwargs
    ...
    Args:
        classifier_type: 分类器类型 ("full_qda", "low_qda_r1", "low_qda_r8", "low_qda_r16", "low_qda_r32", "sgd_linear", "lda")
        stats_dict: 高斯统计量字典
        device: 计算设备
        **kwargs: 分类器特定参数
...
- **Low-rank QDA**: low_rank=True, rank=[1,8,16,32] (分别测试)
...
results = {
    "build_time": {
        ...
        "low_qda_r1": [list of times for each class count],
        "low_qda_r8": [list of times for each class count],
        "low_qda_r16": [list of times for each class count],
        "low_qda_r32": [list of times for each class count],
    ...
    "memory_usage": {
        ...
        "low_qda_r1": [list of memory usage for each class count],
        "low_qda_r8": [list of memory usage for each class count],
        "low_qda_r16": [list of memory usage for each class count],
        "low_qda_r32": [list of memory usage for each class count],
...
### 可视化特点
- **多子图布局**: 构建时间、推理时间、内存使用分别展示
- **误差棒**: 显示标准差或置信区间
- **对数坐标**: 构建时间使用对数坐标以更好展示差异
- **颜色编码**: 每个分类器使用不同颜色
- **图例说明**: 清晰标注各分类器类型
...
### 输出文件
- **结果数据**: `efficiency_results_{model_name}.npz`
- **可视化图表**:
...
```bash
# 小规模测试
python exp_efficiency_comparison.py --class_counts 10 20 50 --num_repeats 1

# 中等规模测试
python exp_efficiency_comparison.py --class_counts 50 100 200 --num_repeats 2

# 完整实验
python exp_efficiency_comparison.py --class_counts 50 100 200 500 1000 --num_repeats 3
```
...
2. **运行实验**:
   ...
   # 自定义参数
   python exp_efficiency_comparison.py --model vit-b-p16 --gpu 1 --num_shots 64
   ```
...
3. **查看结果**:
   ...
   # 查看实验日志
   cat 实验结果保存/效率对比实验/experiment_log.txt
   ```

### 参数说明
- `--model`: 模型名称，支持vit-b-p16, vit-b-p16-clip等
- `--gpu`: GPU编号，默认为0
- `--num_shots`: 每类样本数，默认为128
- `--class_counts`: 类别数量列表，默认为[50, 100, 200, 500, 1000]
- `--num_repeats`: 重复实验次数，默认为3
- `--output_dir`: 结果保存目录，默认为"实验结果保存/效率对比实验"
