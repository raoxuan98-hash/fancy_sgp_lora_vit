...
def example_data_loading():
    ...
    for task_id in range(manager.nb_tasks):
        ...
        try:
            ...
            try:
                train_length = len(train_dataset)  # type: ignore
            except:
                train_length = 'unknown'
            print(f"   ✅ 训练集加载成功: {train_length} 个样本")
            
            # 加载测试集
            test_dataset = manager.get_subset(task_id, source="test", mode="test")
            try:
                test_length = len(test_dataset)  # type: ignore
            except:
                test_length = 'unknown'
            ...
            # 获取第一个样本查看
            try:
                train_sample_len = len(train_dataset)  # type: ignore
                test_sample_len = len(test_dataset)  # type: ignore
            except:
                train_sample_len = 0
                test_sample_len = 0
