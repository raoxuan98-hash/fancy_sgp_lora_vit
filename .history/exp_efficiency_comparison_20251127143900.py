...
"""
计算效率对比实验
比较Full-rank QDA, Low-rank QDA (rank-1,8,16,32), SGD-based linear classifier, LDA的计算效率
"""
# In[]
...
def initialize_results_dict(classifier_types: List[str]) -> Dict[str, Dict[str, List[float]]]:
    """初始化结果字典"""
    return {
        "build_time": {ct: [] for ct in classifier_types},
        "inference_time": {ct: [] for ct in classifier_types},
        "memory_usage": {ct: [] for ct in classifier_types}
    }
...
def run_experiment_for_class_count(
    ...
    for classifier_type in classifier_types:
        ...
        for repeat in range(num_repeats):
            ...
            # 测量构建时间
            build_time, classifier, memory_usage = measure_build_time(
                classifier_type, subset_stats, device=device)
            build_times.append(build_time)
            memory_usages.append(memory_usage["allocated_mb"])
        ...
        # 计算平均值（排除无穷大值）
        valid_build_times = [t for t in build_times if t != float('inf')]
        valid_inference_times = [t for t in inference_times if t != float('inf')]
        valid_memory_usages = [m for m in memory_usages if m != float('inf')]
        
        avg_build_time = np.mean(valid_build_times) if valid_build_times else float('inf')
        avg_inference_time = np.mean(valid_inference_times) if valid_inference_times else float('inf')
        avg_memory_usage = np.mean(valid_memory_usages) if valid_memory_usages else float('inf')
        
        results["build_time"][classifier_type].append(avg_build_time)
        results["inference_time"][classifier_type].append(avg_inference_time)
        results["memory_usage"][classifier_type].append(avg_memory_usage)
        
        logger.info(f"  平均构建时间: {avg_build_time:.4f}s")
