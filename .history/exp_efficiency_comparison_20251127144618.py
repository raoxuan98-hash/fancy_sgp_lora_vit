...
def save_results(results: Dict, class_counts: List[int], model_name: str, output_dir: str):
    ...
    # 保存为NPZ文件
    save_path = os.path.join(output_dir, f"inference_results_{model_name}.npz")
    np.savez(save_path,
             class_counts=class_counts,
             inference_time=results["inference_time"])
    
    logger.info(f"推理时间结果已保存到: {save_path}")
    ...
    logger.info(f"CSV推理时间结果已保存到: {csv_path}")

def load_results(model_name: str, output_dir: str) -> Tuple[Optional[Dict], Optional[List[int]]]:
    """读取预先保存的实验结果"""
    # 尝试读取NPZ文件
    npz_path = os.path.join(output_dir, f"inference_results_{model_name}.npz")
    
    if os.path.exists(npz_path):
        logger.info(f"从NPZ文件读取结果: {npz_path}")
        ...
        # 获取推理时间结果
        inference_time = data['inference_time'].item()
    ...
    # 如果NPZ文件不存在，尝试读取CSV文件
    csv_path = os.path.join(output_dir, f"inference_results_{model_name}.csv")
    
    if os.path.exists(csv_path):
        logger.info(f"从CSV文件读取结果: {csv_path}")
        ...
        with open(csv_path, 'r') as f:
            ...
            for line in f:
                ...
                if len(parts) >= 3:
                    ...
                    if classifier_type in results["inference_time"]:
                        results["inference_time"][classifier_type].append(inference_time)
    ...
    logger.warning(f"未找到预先保存的结果文件: {npz_path} 或 {csv_path}")
    return None, None

# In[]
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='分类器推理时间效率对比实验')
    ...
    parser.add_argument('--load_only', action='store_true',
                        help='只加载预先保存的结果，不运行新实验')
    parser.add_argument('--plot_only', action='store_true',
                        help='只生成图表，需要预先保存的结果')
    ...
    # 设置日志文件
    log_file = os.path.join(args.output_dir, "inference_experiment_log.txt")
    file_handler = logging.FileHandler(log_file)
    file_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
    logger.addHandler(file_handler)
    
    logger.info("开始推理时间效率对比实验")
    ...
    # 尝试加载预先保存的结果
    results, loaded_class_counts = load_results(args.model, args.output_dir)
    
    if results is not None and (args.load_only or args.plot_only):
        logger.info("使用预先保存的结果")
        class_counts = loaded_class_counts
    else:
        # 运行新实验
        logger.info("运行新的推理时间实验")
    ...
    # 生成可视化图表
    if not args.load_only or args.plot_only:
        if class_counts is not None and results is not None:
            plot_inference_time_comparison(
                class_counts, results["inference_time"],
                save_path=os.path.join(args.output_dir, "inference_time_comparison.png")
            )
            
            logger.info("推理时间对比图已生成")
    
    # 设置全局变量，方便后期绘图
    global inference_results, inference_class_counts
    inference_results = results
    inference_class_counts = class_counts if class_counts is not None else []
