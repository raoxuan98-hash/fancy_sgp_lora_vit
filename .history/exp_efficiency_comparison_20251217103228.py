...
def run_efficiency_experiment(
    ...
    classifier_types: List[str] = ["full_qda", "low_qda_r1", "low_qda_r4",  "low_qda_r16", "low_qda_r64", "low_qda_r128", "sgd_linear", "lda"],
    num_repeats: int = 3,
...
def plot_inference_time_comparison(
    ...
    """绘制优化的推理时间对比图（符合IEEE单栏图表规范），仅包含 r=1,8,32 的 LoRA-Rank QDA"""
    # IEEE单栏图片标准尺寸 (3.5英寸宽)
    ...
    # 只保留需要绘制的方法
    methods_to_plot = set(styles.keys())  # 自动包含 full_qda, sgd_linear, lda + r=1,8,32
    ...
    plt.ylim(0.005, 2.5)
    plt.yscale('log')  # 注意：保留对数尺度以便清晰展示数量级差异（你原注释说“已移除”，但代码仍保留；此处根据常规做法保留）
    plt.xscale('log')
    # 坐标轴标签
    ...
    # 紧凑布局，为右侧图例留空间
    plt.tight_layout()
    plt.subplots_adjust(right=0.72)  # 为右侧图例预留足够空间
