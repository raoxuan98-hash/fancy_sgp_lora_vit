...
def initialize_results_dict(classifier_types: List[str]) -> Dict[str, Dict[str, List[float]]]:
    """初始化结果字典，包含推理和构建时间的均值与标准差"""
    metrics = ["inference_time_mean", "inference_time_std", "build_time_mean", "build_time_std"]
    return {
        metric: {ct: [] for ct in classifier_types} for metric in metrics
    }
...
def plot_inference_time_comparison(
    ...
    """绘制优化的推理时间对比图（使用 Mean 数据）"""
    inference_means = inference_results.get("inference_time_mean", inference_results.get("inference_time", {})) # 兼容旧格式

    # IEEE单栏图片标准尺寸 (3.5英寸宽)
    plt.figure(figsize=(3.5, 2.6))
    
    # 仅保留 r=1, r=8, r=32 的 low-rank QDA
    low_qda_colors = {
...
def save_results(results: Dict[str, Dict[str, List[float]]], class_counts: List[int],
    ...
    with open(csv_path, 'w') as f:
        # 写入表头
        f.write("classifier_type,class_count,inference_time_mean,inference_time_std,build_time_mean,build_time_std\n")
        ...
        for classifier_type in classifier_types:
            for i, class_count in enumerate(class_counts):
                ...
                f.write(f"{classifier_type},{class_count},{inf_mean},{inf_std},{build_mean},{build_std}\n")
...
def load_results(model_name: str, output_dir: str = "实验结果保存") -> Tuple[Optional[Dict], Optional[List[int]]]:
    ...
    # 临时存储，格式：temp_data[classifier][class_count] = {metric: val}
    temp_data = {}
    ...
    with open(csv_path, 'r') as f:
        ...
        for line in lines[1:]:
            ...
            c_type, c_count, i_mean, i_std, b_mean, b_std = parts
            c_count = int(c_count)
    ...
    for c_type in temp_data:
        for metric in metrics:
            ...
            for cc in class_counts:
                results[metric][c_type].append(temp_data[c_type][cc][metric])
...
def parse_args():
    ...
    parser.add_argument('--classifier_types', type=str, nargs='+',
                        default=["full_qda", "low_qda_r1", "low_qda_r8", "low_qda_r32", "sgd_linear", "lda"],
                        help='测试的分类器类型列表')
