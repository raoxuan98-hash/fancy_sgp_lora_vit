...
包含：构建时间、推理时间、吞吐量 (Throughput) 的均值与标准差统计
"""
# In[]
...
def initialize_results_dict(classifier_types: List[str]) -> Dict[str, Dict[str, List[float]]]:
    ...
    metrics = [
        ...
        "throughput_mean", "throughput_std"  # 新增吞吐量统计
    ]
    return {
...
def run_experiment_for_class_count(
    ...
    for classifier_type in classifier_types:
        ...
        results["throughput_mean"][classifier_type].append(float(t_mean)) # 记录吞吐量
        results["throughput_std"][classifier_type].append(float(t_std))   # 记录吞吐量
...
def save_results(results: Dict[str, Dict[str, List[float]]], class_counts: List[int],
    ...
    with open(csv_path, 'w') as f:
        # 写入表头，增加 throughput_mean 和 throughput_std
...
def load_results(model_name: str, output_dir: str = "实验结果保存") -> Tuple[Optional[Dict], Optional[List[int]]]:
    ...
    with open(csv_path, 'r') as f:
        ...
        for line in lines[1:]:
            ...
            if len(parts) < 8: continue # 确保列数足够
