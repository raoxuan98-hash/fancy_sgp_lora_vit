...
def parse_log_file(log_path: str) -> Dict:
    ...
    try:
        ...
        # 提取最终任务准确率
        final_task_pattern = r'─ Final Task Accuracy \(%\) ─\s*\n(?:.*:\s*([\d.]+)%\s*\n)+'
        final_task_matches = re.findall(final_task_pattern, content, re.MULTILINE)
        
        # 提取平均准确率
        avg_task_pattern = r'─ Average Accuracy Across Tasks \(%\) ─\s*\n(?:.*:\s*([\d.]+)%\s*\n)+'
        avg_task_matches = re.findall(avg_task_pattern, content, re.MULTILINE)
        
        # 提取变体名称和对应的准确率
        variant_pattern = r'(\S+(?:\s+\+\s+\S+)*)\s*:\s*([\d.]+)%'
        
        # 查找所有准确率报告部分
        accuracy_sections = re.findall(r'─ Final Task Accuracy \(%\) ─\s*\n((?:.*:\s*[\d.]+%\s*\n)+)', content, re.MULTILINE)
        ...
        avg_sections = re.findall(r'─ Average Accuracy Across Tasks \(%\) ─\s*\n((?:.*:\s*[\d.]+%\s*\n)+)', content, re.MULTILINE)
        if avg_sections:
        ...
        if summary_matches:
            result['best_final'] = summary_matches[-1][0]
            result['best_average'] = summary_matches[-1][1]
