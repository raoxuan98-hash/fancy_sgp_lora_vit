...
def parse_log_file(log_path: str) -> Dict:
    ...
    try:
        ...
        # 特别提取Aggregated Final Task Accuracy中的SeqFT + attention_transform + QDA
        aggregated_pattern = r'Aggregated Final Task Accuracy.*?\n((?:\s*[^:]+:\s*[\d.]+%\s*±\s*[\d.]+%\s*\n)+)'
        aggregated_matches = re.findall(aggregated_pattern, content, re.DOTALL)
        if aggregated_matches:
            for section in aggregated_matches:
                # 查找SeqFT + attention_transform + QDA的准确率
                seqft_attention_qda_pattern = r'SeqFT\s+\+\s+attention_transform\s+\+\s+QDA\s*:\s*([\d.]+)%\s*±\s*[\d.]+%'
                match = re.search(seqft_attention_qda_pattern, section)
                if match:
                    result['final_accuracies']['SeqFT + attention_transform + QDA'] = float(match.group(1))
