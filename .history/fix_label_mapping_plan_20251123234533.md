...
def _create_incremental_splits(self) -> None:
    ...
    for dataset_idx, dataset in enumerate(self.datasets):
        ...
        for split_idx, class_indices in enumerate(class_splits):
            ...
            # 对于增量分割，每个子集都是独立的，标签从0开始
            # 所以全局偏移就是前面所有子集的类别数总和
    ...
    # 重新构建global_class_names
    self.global_class_names = []
    for dataset in self.datasets:
        self.global_class_names.extend(dataset['class_names'])
```
...
1. 修改 `BalancedCrossDomainDataManagerCore._create_incremental_dataset` 方法
2. 修改 `CrossDomainSimpleDataset` 类以支持标签映射
3. 更新 `get_subset` 方法以正确处理增量子集
4. 修改 `_create_incremental_splits` 方法中的全局偏移计算
5. 测试修复后的代码

## 预期结果

修复后，当 `enable_incremental_split=True` 时：
1. 每个增量子集的标签将从0开始连续编号
2. 标签范围将与子集的类别数匹配
3. 不会再出现"Label X exceeds class_names length Y"错误
4. 模型可以正常进行增量学习