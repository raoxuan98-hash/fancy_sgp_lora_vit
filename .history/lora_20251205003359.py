...
class NullSpaceViT(nn.Module):
    ...
    def get_param_groups(self):
        ...
        # 添加最终norm的权重（如果可训练）
        if hasattr(self.lora_vit, 'norm') and hasattr(self.lora_vit.norm, 'weight') and self.lora_vit.norm.weight.requires_grad:
    ...
    def merge_lora_weights(self):
        """
        NullSpaceViT不使用LoRA权重，但为了兼容性提供此函数
        在全参数微调模式下，此函数不做任何操作
        """
        # NullSpaceViT直接训练原始权重，不需要合并LoRA权重
        # 此函数仅为保持与其他LoRA类型的兼容性而存在
        pass
