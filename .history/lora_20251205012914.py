...
class NullSpaceViT(nn.Module):
    ...
    def __init__(
        ...
        full_finetune: bool = False,  # 新增参数：是否启用全量微调模式
    ):
        super().__init__()
        # ---------- 1️⃣ 记录要适配的模块 ----------
        self.lora_modules = nn.ModuleDict()
        self.full_finetune = full_finetune  # 保存全量微调标志
        ...
        # 在全量微调模式下，final_norm的weight也是可训练的
        for n, p in self.lora_vit.norm.named_parameters():
            if "weight" in n:
                p.requires_grad = self.full_finetune  # 根据full_finetune决定是否可训练
            else:  # bias
                p.requires_grad = False
    ...
    def summary(self) -> str:
        ...
        mode = "Full Fine-tune + NullSpace" if self.full_finetune else "NullSpace"
        lines = [
            f"=== {mode}ViT Summary ===",
        ...
        if hasattr(self.lora_vit, 'norm') and self.lora_vit.norm is not None and hasattr(self.lora_vit.norm, 'weight') and self.lora_vit.norm.weight is not None:
            norm_status = "trainable" if self.lora_vit.norm.weight.requires_grad else "frozen"
            lines.append(f"  • final_norm_weight (shape={tuple(self.lora_vit.norm.weight.shape)}, status={norm_status})")
        else:
            lines.append(f"  • final_norm_weight (shape=unknown)")
            
        lines.append(f"Projection enabled : {self.use_projection}")
        lines.append(f"Full fine-tune mode : {self.full_finetune}")
        return "\n".join(lines)
