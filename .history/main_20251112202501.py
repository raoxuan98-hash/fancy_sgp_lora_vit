...
def _pretty_print_aggregate(aggregate_results: dict) -> None:
    ...
    per_task_trends = aggregate_results.get('per_task_accuracy_trends', {})
    ...
    if per_task_trends:
        print('\n===== Per-Task Accuracy Trends (Mean ± Std) =====')
        for variant, trend_data in per_task_trends.items():
            means = trend_data.get('means', [])
            stds = trend_data.get('stds', [])
            if means and stds:
                trend_str = " → ".join([f"{m:.2f}%±{s:.2f}%" for m, s in zip(means, stds)])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    basic.add_argument('--cross_domain_datasets', type=str, nargs='+', default=['imagenet-r', 'caltech-101', 'dtd', 'eurosat_clip', 'fgvc-aircraft-2013b-variants102', 'food-101', 'mnist', 'oxford-flower-102', 'oxford-iiit-pets', 'stanford-cars'], help='List of datasets for cross-domain experiments')
