...
def build_parser() -> argparse.ArgumentParser:
    ...
    basic.add_argument('--cross_domain_datasets', type=str, nargs='+', default=['imagenet-r', 'caltech-101', 'dtd', 'eurosat_clip', 'fgvc-aircraft-2013b-variants102', 'food-101', 'mnist', 'oxford-flower-102', 'oxford-iiit-pets', 'cars196_224'], help='List of datasets for cross-domain experiments')
    basic.add_argument('--num_shots', type=int, default=16, help='Number of samples per class for few-shot learning. If > 0, randomly sample num_shots samples per class.')
    basic.add_argument('--num_samples_per_task_for_evaluation', type=int, default=1000, help='Number of samples per task for evaluation. If > 0, randomly sample this many samples from each test task for fast evaluation.')
    ...
    comp.add_argument('--compensator_types', type=str, nargs='+', default=['SeqFT', 'SeqFT + linear', 'SeqFT + weaknonlinear', 'SeqFT + Hopfield'],
                     choices=['SeqFT', 'SeqFT + linear', 'SeqFT + Hopfield'],
