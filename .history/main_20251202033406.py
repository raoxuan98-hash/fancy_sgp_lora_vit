...
def build_parser() -> argparse.ArgumentParser:
    ...
    basic.add_argument('--user', type=str, default='2025-11-29-main', choices=['authors'], help='User identifier (currently unused).')
    ...
    cls.add_argument('--init_cls', type=int, default=50, help='Number of classes in the first task.')
    cls.add_argument('--increment', type=int, default=50, help='Number of new classes added per task.')
    ...
    model.add_argument('--vit_type', type=str, default='vit-b-p16-dino', choices=['vit-b-p16', 'vit-b-p16-dino', 'vit-b-p16-mae', 'vit-b-p16-clip', 'vit-b-p16-mocov3'], help='ViT backbone variant.')
    ...
    train_grp.add_argument('--seed_list', nargs='+', type=int, default=[1993, 199], help='Random seeds for multiple runs.')
    train_grp.add_argument('--iterations', type=int, default=10, help='Training iterations per task.')
    train_grp.add_argument('--warmup_ratio', type=int, default=0.01, help='Warm‑up ratio for learning rate schedule.')
    ...
    train_grp.add_argument('--update_teacher_each_task', action="store_true", default=False, help='If set, update the teacher network after each task.')
    ...
    train_grp.add_argument('--distillation_transform', type=str, default='weaknonlinear', help='Distillation head transform (identity / linear / weaknonlinear).')
    ...
    gda.add_argument('--qda_reg_alpha1', type=float, default=0.10, help='QDA regularisation alpha 1.')
    ...
    aux.add_argument('--auxiliary_data_size', type=int, default=4096, help='Number of samples drawn from the auxiliary dataset each epoch.')
    aux.add_argument('--feature_combination_type', type=str, default="aux_only", choices=['combined', 'aux_only', 'current_only'], help='Type of feature combination.')
    ...
    comp.add_argument('--compensator_types', type=str, nargs='+', default=['SeqFT', 'SeqFT + rff', 'SeqFT + linear'],
                     choices=['SeqFT', 'SeqFT + linear', 'SeqFT + weaknonlinear', 'SeqFT + Hopfield', 'SeqFT + rff'],
                     help='Types of compensators to use. Default is all four types.')
    comp.add_argument('--hopfield_temp', type=float, default=0.1, help='Temperature parameter for Hopfield attention compensator.')
    comp.add_argument('--hopfield_topk', type=int, default=200, help='Top-k parameter for Hopfield attention compensator.')
