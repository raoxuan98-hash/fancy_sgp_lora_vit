...
def set_smart_defaults(ns):
    # 如果是跨域实验，直接返回，不使用smart_defaults
    # if ns.cross_domain:
    #     return ns
...
def build_parser() -> argparse.ArgumentParser:
    ...
    basic.add_argument('--user', type=str, default='2025-11-04-test', choices=['authors'], help='User identifier (currently unused).')
    basic.add_argument('--test', action='store_true', default=False, help='If set, run a quick test with reduced settings.')
    basic.add_argument('--cross_domain', action="store_true", default=True)
    '''order 1 datasets'''
    ...
    inc.add_argument('--enable_incremental_split', action="store_true", default=False, help='Enable incremental split for cross-domain datasets. If set, each dataset will be split into multiple incremental subsets.')
    inc.add_argument('--num_incremental_splits', type=int, default=1, help='Number of incremental splits per dataset when enable_incremental_split is True.')
    ...
    model.add_argument('--vit_type', type=str, default='vit-b-p16-clip', choices=['vit-b-p16', 'vit-b-p16-dino', 'vit-b-p16-mae', 'vit-b-p16-clip', 'vit-b-p16-mocov3'], help='ViT backbone variant.')
    ...
    train_grp.add_argument('--iterations', type=int, default=1, help='Training iterations per task.')
    ...
    train_grp.add_argument('--lrate', type=float, default=1e-8, help='Learning rate.')
    ...
    # 分类器参数
    cls.add_argument('--classifier_types', type=str, nargs='+', default=['lda', 'qda', 'sgd'], choices=['lda', 'qda', 'sgd', 'ls'], help='Types of classifiers to build. Default is both LDA and QDA.')
