...
def main(args):
    """within-domain实验主函数"""
    # 设置cross_domain为False，确保使用within-domain数据管理器
    args['cross_domain'] = False
    results = train(args)

def build_parser() -> argparse.ArgumentParser:
    """构建within-domain实验的参数解析器"""
    parser = argparse.ArgumentParser(description='Within-Domain Incremental Learning Experiments')
    
    # 基本参数
    basic = parser.add_argument_group('basic', 'General / high‑level options')
    basic.add_argument('--dataset', type=str, default='cars196_224', 
                      choices=['imagenet-r', 'cifar100_224', 'cub200_224', 'cars196_224', 
                              'caltech101_224', 'oxfordpet37_224', 'food101_224', 'resisc45_224'], 
                      help='Dataset to use for within-domain experiment')
