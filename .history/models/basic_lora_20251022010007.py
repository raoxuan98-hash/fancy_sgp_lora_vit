...
class BaseLoRAViT(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        for _, module in self.lora_modules.items():
            ...
            # 重新初始化 DoRA magnitude 参数
            if hasattr(module, 'magnitude'):
                with torch.no_grad():
                    if isinstance(module, OriginalDoRA):
                        # 重新计算原始权重的范数作为 magnitude 的初始值
                        weight = module.weight_directions.data
                        weight_norm = weight.norm(p=2, dim=1, keepdim=True) + 1e-8
                        module.magnitude.data = weight_norm.clone()
                    else:
                        # 对于其他变体，使用 Xavier 初始化
                        nn.init.xavier_uniform_(module.magnitude)

    def reset_parameters_svd(self) -> None:
        """SVD-based 参数初始化 - 重新初始化所有LoRA/DoRA参数"""
        for _, module in self.lora_modules.items():
            # 获取基础权重矩阵
            ...
            # 重新初始化 magnitude（对于 DoRA）
            if hasattr(module, 'magnitude'):
                with torch.no_grad():
                    if isinstance(module, OriginalDoRA):
                        weight_norm = W.norm(p=2, dim=1, keepdim=True) + 1e-8
                        module.magnitude.data = weight_norm.clone()
                    else:
                        nn.init.xavier_uniform_(module.magnitude)
