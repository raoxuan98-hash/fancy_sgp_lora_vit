...
class OriginalLoRA(nn.Module):
    def __init__(
        ...
        # LoRA 参数
        self.alphas = nn.Parameter(torch.tensor(0.8))
    ...
    def forward(self, x):
        if self.lora_active:
            ...
            result += self.alphas * (x @ self.lora_A.T @ self.lora_B.T)
            return result
        else:
    ...
    def merge_lora_weights(self, lora_active: bool = True) -> None:
        with torch.no_grad():
            lora_delta = self.alphas * (self.lora_B @ self.lora_A)
...
class BaseLoRAViT(nn.Module):
    ...
    def reset_parameters_svd(self) -> None:
        ...
        for _, module in self.lora_modules.items():
            ...
            U, S, Vh = torch.svd_lowrank(W, q=min(q, min(W.shape)-1), niter=niter)
            U_r = U[:, :self.r]  # (out_features, r)
            S_r = S[:self.r]     # (r,)
            V_r = Vh[:, :self.r] # (in_features, r)
            
            # 初始化 lora_A 和 lora_B
            module.lora_A.data = V_r.T.clone()  # (r, in_features)
            module.lora_B.data = (U_r * S_r.unsqueeze(0)).clone()  # (out_features, r)
