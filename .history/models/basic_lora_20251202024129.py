...
class BaseLoRAViT(nn.Module):
    def __init__(
        ...
        for idx, blk in enumerate(vit_model.blocks):
            ...
            # ---- QKV ----
            new_qkv = lora_class(blk.attn.qkv, r, lora_scale)
            blk.attn.qkv = new_qkv
            self.lora_modules[f"block_{idx}_attn_qkv"] = new_qkv

            # ---- MLP fc1 ----
            new_fc1 = lora_class(blk.mlp.fc1, r, lora_scale)
            blk.mlp.fc1 = new_fc1
            self.lora_modules[f"block_{idx}_mlp_fc1"] = new_fc1

            # ---- MLP fc2 ----
            new_fc2 = lora_class(blk.mlp.fc2, r, lora_scale)
            blk.mlp.fc2 = new_fc2
    ...
    def reset_parameters_svd(self) -> None:
        ...
        for _, module in self.lora_modules.items():
            ...
            # print(Vh.shape)
            U, S, Vh = torch.linalg.svd(W.cuda())
            print(Vh.shape)
            # module.lora_A.data[:] =  Vh[:self.r, :]
