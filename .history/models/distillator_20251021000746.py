...
class Distiller(nn.Module):
    ...
    def update_teacher(self, student_network: nn.Module):
        ...
        if need_reinit and self.head is not None:
            feat_dim = self.head[0].in_features if isinstance(self.head, nn.Sequential) else getattr(self.head, "in_features", None)
            transform_type = (
                "weaknonlinear" if isinstance(self.head, ResidMLP)
                else "linear" if isinstance(self.head, nn.Linear)
                else "identity"
            )
            self.head = self._build_head(feat_dim, transform_type).to(self.device)
