...
class Distiller(nn.Module):
    ...
    def norm_loss(self, student_features, teacher_features):
        norms_t = torch.norm(student_features, p=2, dim=1)
        norms_s = torch.norm(teacher_features, p=2, dim=1)
    ...
    def forward(self, inputs: torch.Tensor, student_features: torch.Tensor) -> torch.Tensor:
        ...
        kd_loss = self.loss_fn(student_features, teacher_features) + self.norm_loss(student_features, teacher_features)
