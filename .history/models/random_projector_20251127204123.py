...
from models.base import RandomNet
...
class RandomProjector(RandomNet):
    ...
    def process_batch(self, inputs, targets, optimizer):
        ...
        # DEBUG: 记录输入和标签的基本信息
        logging.debug(f"[DEBUG] process_batch: inputs.shape={inputs.shape}, targets.shape={targets.shape}")
        logging.debug(f"[DEBUG] process_batch: targets.min={targets.min().item()}, targets.max={targets.max().item()}")
        logging.debug(f"[DEBUG] process_batch: self._known_classes={self._known_classes}, self._total_classes={self._total_classes}")
        ...
        # DEBUG: 记录logits信息
        logging.debug(f"[DEBUG] process_batch: logits.shape={logits.shape}")
        ...
        # DEBUG: 记录处理后的标签和logits信息
        logging.debug(f"[DEBUG] process_batch: new_targets_rel.shape={new_targets_rel.shape}")
        logging.debug(f"[DEBUG] process_batch: new_targets_rel.min={new_targets_rel.min().item()}, new_targets_rel.max={new_targets_rel.max().item()}")
        logging.debug(f"[DEBUG] process_batch: new_logits.shape={new_logits.shape}")
        
        # 检查是否有标签超出new_logits的范围
        valid_targets = new_targets_rel[new_targets_rel >= 0]
        if len(valid_targets) > 0:
            max_valid_target = valid_targets.max().item()
            if max_valid_target >= new_logits.size(1):
                logging.error(f"[ERROR] Label {max_valid_target} exceeds new_logits dimension {new_logits.size(1)}")
                logging.error(f"[ERROR] new_logits.size(1)={new_logits.size(1)}, max_valid_target={max_valid_target}")
                logging.error(f"[ERROR] self._known_classes={self._known_classes}, self._total_classes={self._total_classes}")
