...
class SubspaceLoRA(BaseLearner):
    ...
    def incremental_train(self, data_manager) -> None:
        ...
        # 使用 prev_network 进行特征蒸馏
        if self.use_feature_kd and self.prev_network is not None:
            # 将 prev_network 移到设备上用于蒸馏
            self.prev_network = self.prev_network.to(self._device)
    ...
    def process_batch(self, inputs, targets, optimizer):
        ...
        if self.use_feature_kd and self.prev_network is not None:
            ...
            if self.args["kd_type"] == "feat":
                kd_term = self.gamma_kd * feature_distillation_loss(teacher_feats, feats)
