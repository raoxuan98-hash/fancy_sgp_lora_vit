...
class Drift_Compensator(object):
    ...
    @torch.no_grad()
    def extract_features_before_after(self, model_before, model_after, data_loader, after_only: bool = False):
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        feats_before, feats_after, labels = self.extract_features_before_after(
            model_before, model_after, data_loader, after_only=False)
        ...
        if self.compensate and task_id > 1:
            ...
            feats_before = torch.cat([feats_before, feats_aux_before], dim=0)
            feats_after = torch.cat([feats_after, feats_aux_after], dim=0)
