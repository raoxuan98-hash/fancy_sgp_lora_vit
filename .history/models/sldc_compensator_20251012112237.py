...
class WeakNonlinearTransform:
    """弱非线性变换器，基于ResidMLP"""
    def __init__(self, input_dim: int, hidden_scale: float = 2.0, num_layers: int = 2):
        self.net = ResidMLP(input_dim, hidden_scale, num_layers)
    ...
    def train(self, features_before: torch.Tensor, features_after: torch.Tensor, 
        ...
        for epoch in range(epochs):
            optimizer.zero_grad()
            pred = self.net(X)
            loss = criterion(pred, Y) + 0.01 * self.net.reg_loss()
            loss.backward()
            optimizer.step()
            
            if (epoch + 1) % 10 == 0:
                print(f"非线性变换训练 Epoch {epoch+1}/{epochs}, Loss: {loss.item():.6f}")
...
class Drift_Compensator(object):
    ...
    def compute_nonlinear_transform(self, features_before: torch.Tensor, features_after: torch.Tensor) -> WeakNonlinearTransform:
        """基于前/后特征训练弱非线性变换器"""
        ...
        transform = WeakNonlinearTransform(
            ...
            hidden_scale=self.nonlinear_hidden_scale,
            num_layers=self.nonlinear_num_layers
        )
