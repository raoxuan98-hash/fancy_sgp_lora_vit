...
class QDAClassifier(nn.Module):
    """二次判别分析分类器"""
    def __init__(self, n_classes, feature_dim, use_cholesky=True):
    ...
    def fit(self, stats_dict, priors=None):
        ...
        for i, cid in enumerate(class_ids):
            stat = stats_dict[cid]
            self.means[i] = stat.mean
            cov = stat.cov
            self.covariances[i] = cov
            
            if self.use_cholesky:
                L = cholesky_manual_stable(cov)
                self.cov_cholesky[i] = L
                self.log_dets[i] = 2 * torch.log(torch.diag(L)).sum()
            else:
                self.log_dets[i] = torch.logdet(cov)
            
            self.priors[i] = priors.get(cid, 1.0/len(stats_dict))
        
        self.is_fitted = True
        return self

    def mahalanobis_distance(self, x, class_idx):
        """计算马氏距离: (x-μ)^T Σ^{-1} (x-μ)"""
        diff = x - self.means[class_idx]
        
        if self.use_cholesky:
            L = self.cov_cholesky[class_idx]
            z = torch.linalg.solve_triangular(L, diff.T, upper=False)
            distance = (z ** 2).sum(dim=0)
        else:
            cov_inv = torch.linalg.inv(self.covariances[class_idx])
            distance = (diff @ cov_inv @ diff.T).diag()
    ...
    def log_probability(self, x):
        """计算每个类的对数概率"""
        batch_size = x.size(0)
        log_probs = torch.zeros(batch_size, self.n_classes, device=x.device)
        
        for i in range(self.n_classes):
            mahalanobis = self.mahalanobis_distance(x, i)
            log_probs[:, i] = -0.5 * (mahalanobis + self.log_dets[i]) + torch.log(self.priors[i])
    ...
    def stabilized_forward(self, x, epsilon=1e-6):
        """数值稳定的QDA前向计算"""
        batch_size = x.size(0)
        log_probs = torch.zeros(batch_size, self.n_classes, device=x.device)
        ...
        for i in range(self.n_classes):
            ...
            try:
                z = torch.linalg.solve_triangular(L, diff.T, upper=False)
                mahalanobis = (z ** 2).sum(dim=0)
            except:
                cov = self.covariances[i] + epsilon * torch.eye(self.feature_dim, device=x.device)
                cov_inv = torch.linalg.pinv(cov)
                mahalanobis = (diff @ cov_inv @ diff.T).diag()
            
            log_probs[:, i] = -0.5 * (mahalanobis + self.log_dets[i]) + torch.log(self.priors[i] + epsilon)
...
class Drift_Compensator(object):
    def __init__(self, args):
        ...
        self.use_qda = args.get('use_qda', False)  # 新增：控制是否使用QDA
        ...
        self.feature_dim = None  # 新增：记录特征维度
    ...
    def build_qda_classifiers(self, variants_stats):
        """为所有变体构建QDA分类器"""
        qda_classifiers = {}
        ...
        for variant_name, stats_dict in variants_stats.items():
            ...
            qda = QDAClassifier(n_classes, feature_dim).to(self.device)
            
            # 计算先验概率（可选）
            total_samples = sum([stat.mean.shape[0] for stat in stats_dict.values()])
            priors = {cid: stat.mean.shape[0]/total_samples for cid, stat in stats_dict.items()}
            
            qda.fit(stats_dict, priors)
            qda_classifiers[variant_name] = qda
            
            print(f"[QDA] Built {variant_name} classifier with {n_classes} classes")
        
        return qda_classifiers

    def build_all_classifiers(self, task_id, model_before, model_after, data_loader, use_qda=True):
        """构建所有分类器变体，可选择使用SGD或QDA"""
        ...
        # 1. 构建统计变体
        variants_stats = self.build_all_variants(task_id, model_before, model_after, data_loader)
        
        classifiers = {'stats': variants_stats}
        
        if use_qda:
            # 使用QDA分类器
            print("使用QDA分类器...")
            qda_classifiers = self.build_qda_classifiers(variants_stats)
            classifiers['qda'] = qda_classifiers
            classifiers['linear'] = {}  # 线性分类器为空
        else:
            # 使用SGD训练的线性分类器
            print("使用SGD训练的线性分类器...")
            n_classes_total = sum(len(stats) for stats in variants_stats.values())
            fc_template = nn.Linear(self.feature_dim, n_classes_total).to(self.device)
            linear_classifiers = self.refine_classifiers_from_variants(fc_template)
            classifiers['linear'] = linear_classifiers
            classifiers['qda'] = {}  # QDA分类器为空
    ...
    @torch.no_grad()
    def extract_test_features(self, test_loader, model=None):
        """提取测试特征用于评估"""
        if model is None:
            model = test_loader.model if hasattr(test_loader, 'model') else None
        if model is None:
            raise ValueError("需要提供模型或测试数据加载器关联模型")
    ...
    def evaluate_classifiers(self, test_loader, classifiers, model=None):
        """评估分类器性能"""
        results = {}
        
        # 提取测试特征
        test_features, test_labels = self.extract_test_features(test_loader, model)
        test_features = test_features.to(self.device)
        test_labels = test_labels.to(self.device)
        
        # 评估线性分类器
        if 'linear' in classifiers and classifiers['linear']:
            linear_results = {}
            for name, clf in classifiers['linear'].items():
                with torch.no_grad():
                    logits = clf(test_features)
                    preds = torch.argmax(logits, dim=1)
                    acc = (preds == test_labels).float().mean().item()
                    linear_results[name] = acc
                    print(f"Linear {name}: {acc:.4f}")
            results['linear'] = linear_results
        
        # 评估QDA分类器
        if 'qda' in classifiers and classifiers['qda']:
            qda_results = {}
            for name, clf in classifiers['qda'].items():
                with torch.no_grad():
                    preds = clf.predict(test_features)
                    acc = (preds == test_labels).float().mean().item()
                    qda_results[name] = acc
                    print(f"QDA {name}: {acc:.4f}")
            results['qda'] = qda_results
...
if __name__ == "__main__":
    ...
    args = {
        ...
        'compensate': True,
        'use_weaknonlinear': True,
        'use_qda': True  # 控制使用QDA还是SGD
    }
    
    compensator = Drift_Compensator(args)
    ...
    class MockModel(nn.Module):
        def __init__(self, feature_dim=10):
            super().__init__()
            self.feature_dim = feature_dim
            self.fc = nn.Linear(784, feature_dim)
    ...
    class MockDataLoader:
        def __init__(self):
            self.data = [(torch.randn(32, 1, 28, 28), torch.randint(0, 3, (32,))) for _ in range(10)]
    ...
    model_before = MockModel(10)
    model_after = MockModel(10)
    data_loader = MockDataLoader()
    
    # 测试构建分类器
    classifiers = compensator.build_all_classifiers(
        task_id=1, 
        model_before=model_before, 
        model_after=model_after, 
        data_loader=data_loader,
        use_qda=True  # 可以在这里覆盖默认设置
    )
    
    print("分类器构建完成！")
    if classifiers['qda']:
        print(f"构建了 {len(classifiers['qda'])} 个QDA分类器")
    if classifiers['linear']:
        print(f"构建了 {len(classifiers['linear'])} 个线性分类器")
