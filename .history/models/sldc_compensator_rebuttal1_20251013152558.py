...
class Drift_Compensator(object):
    def __init__(self, args):
        ...
        # === DPCR 关键超参（可调） ===
        self.energy = args.get('dpcr_energy', 0.95)  # 类行空间累计能量阈值（0~1），默认 95%
        self.r_cap  = args.get('dpcr_r_cap', 256)    # 投影秩的上限，防止维度过大
    ...
    def refine_classifiers_from_variants(self, fc: nn.Module, epochs: int = 5, method: str = "qda",
        ...
        """
        对 self.variants 中每个分布训练/构建一个分类器。
        method: 'sgd' | 'qda' | 'lda'
        """
        ...
        if method == "sgd":
            ...
            for name, stats in self.variants.items():
                ...
                try:
                    ...
                    # 简单运行一次前向，检查设备一致性
                    with torch.no_grad():
                        test_input = torch.randn(2, clf.global_mean.size(0), device=self.device)
                        _ = clf(test_input)
