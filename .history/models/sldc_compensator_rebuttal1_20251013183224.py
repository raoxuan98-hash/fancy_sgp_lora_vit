...
class RegularizedGaussianClassifier(nn.Module):
    def __init__(self, stats_dict: Dict[int, 'GaussianStatistics'], 
        ...
        for cid in self.class_ids:
            stat = stats_dict[cid]
            mean_tensor = stat.mean.clone().detach().float().to(self.device)
            cov_tensor = stat.cov.clone().detach().float().to(self.device)
        ...
        # ---------- 确保正定 ----------
        def ensure_positive_definite(cov: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
            cov = cov + eps * torch.eye(cov.size(0), device=cov.device)
            try:
                eigvals = torch.linalg.eigvalsh(cov)
                min_eigval = eigvals.min()
                if min_eigval < 1e-8:
                    cov += (1e-8 - min_eigval) * torch.eye(cov.size(0), device=cov.device)
            except:
        ...
        for cid in self.class_ids:
            cov = self.original_covs[str(cid)].to(self.device)
            cov_reg = regularize_covariance(cov).to(self.device)
            cov_reg = ensure_positive_definite(cov_reg)
            ...
            self.regularized_covs[str(cid)] = nn.Parameter(cov_reg, requires_grad=False)
            self.cov_invs[str(cid)] = nn.Parameter(cov_inv, requires_grad=False)
            self.log_dets[str(cid)] = nn.Parameter(log_det.unsqueeze(0), requires_grad=False)

        # ---------- 共享协方差（LDA 模式） ----------
        if mode == "lda":
            shared_cov = regularize_covariance(self.global_cov).to(self.device)
            shared_cov = ensure_positive_definite(shared_cov)
            with torch.no_grad():
                shared_inv = torch.linalg.pinv(shared_cov)
                sign, logabsdet = torch.linalg.slogdet(shared_cov)
                shared_logdet = logabsdet

            self.shared_cov = nn.Parameter(shared_cov, requires_grad=False)
            self.shared_cov_inv = nn.Parameter(shared_inv, requires_grad=False)
            self.shared_log_det = nn.Parameter(shared_logdet.unsqueeze(0), requires_grad=False)
...
class Drift_Compensator(object):
    ...
    # =====================================================
    #         DPCR 完整流程: TSSP + CIP + RRCR + CN
    # =====================================================
    @torch.no_grad()
    def dpcr_full_compensation(self,
                               old_stats_dict: Dict[int, GaussianStatistics],
                               new_stats_dict: Dict[int, GaussianStatistics],
        ...
        完整 DPCR 流程：
        1. TSSP: 全局线性映射 P
        2. CIP: 类别行空间投影 P_c = P U_c^r U_c^r^T
        3. RRCR: 岭回归分类器重建
        4. CN: 类别权重归一化
        返回分类器权重矩阵 W_t (d, C)
        """
        ...
        d = feats_before.size(1)

        # ---------- Step 1. TSSP ----------
        print("[DPCR] Step 1: Computing global task-wise projection P (TSSP)...")
        P = self.compute_linear_transform(feats_before, feats_after, normalize=True).to(device)

        # ---------- Step 2. CIP ----------
        print("[DPCR] Step 2: Building category subspace projectors (CIP)...")
        proj_basis = self._compute_class_projectors(feats_before, labels)
        P_c_dict = {}
        for cid, U_r in proj_basis.items():
            U_r = U_r.to(device)
            P_c = (P @ (U_r @ U_r.T)).contiguous()
            P_c_dict[cid] = P_c

        # ---------- Step 3. Apply to old stats (TSSP + CIP) ----------
        compensated_old_stats = {}
        for cid, stat in old_stats_dict.items():
            stat = stat.to(device)
            P_c = P_c_dict.get(cid, P)
            cov_new = P_c.T @ stat.cov @ P_c
            mean_new = stat.mean @ P_c
            compensated_old_stats[cid] = GaussianStatistics(mean_new.cpu(), cov_new.cpu(), stat.reg)

        # ---------- Step 4. Aggregate (old + new) ----------
        all_stats = {**compensated_old_stats, **new_stats_dict}
        C = len(all_stats)
        print(f"[DPCR] Step 3: Compensated {len(old_stats_dict)} old classes, added {len(new_stats_dict)} new.")

        # ---------- Step 5. RRCR (Ridge Reconstruction) ----------
        print("[DPCR] Step 4: Closed-form ridge regression reconstruction (RRCR)...")
        Phi_sum = torch.zeros(d, d, device=device)
        H_sum = torch.zeros(d, C, device=device)

        for idx, (cid, stat) in enumerate(all_stats.items()):
            mu = stat.mean.to(device)
            Phi = stat.cov.to(device)
            Phi_sum += Phi
            H_sum[:, idx] = mu

        # Ridge closed-form
        W_t = torch.linalg.solve(Phi_sum + gamma * torch.eye(d, device=device), H_sum)
        # ---------- Step 6. CN ----------
        W_t = F.normalize(W_t, dim=0)
        print(f"[DPCR] Step 5: Normalized classifier weights, shape = {W_t.shape}")
        return W_t.cpu()
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        ...
        if self.compensate and task_id > 1:
            ...
            # ============ 完整 DPCR：TSSP + CIP + RRCR + CN ============
            if "DPCR" in self.variants:
                try:
                    W_t_dpcr = self.dpcr_full_compensation(
                        old_stats_dict=self.variants["DPCR"],
                        new_stats_dict=copy.deepcopy(stats),
                        feats_before=feats_before,
                        feats_after=feats_after,
                        labels=labels,
                        gamma=self.gamma
                    )
                    self.dpcr_classifier_weight = W_t_dpcr  # 缓存解析分类器
                    print(f"[DPCR] Full reconstruction done. W_t shape = {W_t_dpcr.shape}")
                except Exception as e:
                    print(f"[DPCR] Full reconstruction failed: {e}")
                    self.dpcr_classifier_weight = None

            # ============ DPCR + ADE ============
            if "DPCR + ADE" in self.variants and W is not None:
                try:
                    W_t_dpcr_ade = self.dpcr_full_compensation(
                        old_stats_dict=self.variants["DPCR + ADE"],
                        new_stats_dict=copy.deepcopy(stats),
                        feats_before=feats_before_combined if aux_loader else feats_before,
                        feats_after=feats_after_combined if aux_loader else feats_after,
                        labels=labels,
                        gamma=self.gamma
                    )
                    self.dpcr_classifier_weight_ade = W_t_dpcr_ade
                    print(f"[DPCR+ADE] Reconstruction done. W_t shape = {W_t_dpcr_ade.shape}")
                except Exception as e:
                    print(f"[DPCR+ADE] Reconstruction failed: {e}")
                    self.dpcr_classifier_weight_ade = None
        ...
        for name, stats in self.variants.items():
            ...
            try:
                ...
                # DPCR 系列 -> QDA（类内各自协方差）
                    ...
                    W_t = getattr(self, 'dpcr_classifier_weight' if "ADE" not in name else 'dpcr_classifier_weight_ade', None)
                    if W_t is not None:
                        in_dim, out_dim = W_t.shape
                        clf = nn.Linear(in_dim, out_dim, bias=False).to(self.device)
                        clf.weight.data = W_t.T.to(self.device)
                        out[name] = clf
                        print(f"[INFO] {name}: Analytic classifier loaded ({out_dim} classes)")
