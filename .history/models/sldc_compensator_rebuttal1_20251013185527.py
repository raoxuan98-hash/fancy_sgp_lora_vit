...
class RegularizedGaussianClassifier(nn.Module):
    def __init__(self, 
        ...
        for cid in self.class_ids:
            stat = stats_dict[cid]
            means.append(stat.mean.to(torch.float32))
            covs.append(stat.cov.to(torch.float32))
        means = torch.stack(means).to(self.device)  # [C, D]
        covs = torch.stack(covs).to(self.device)    # [C, D, D]

        # ===== 计算全局统计量 =====
        self.global_mean = nn.Parameter(means.mean(dim=0).cpu(), requires_grad=False)
        self.global_cov = nn.Parameter(covs.mean(dim=0).cpu(), requires_grad=False)

        # ===== 批量正则化协方差 =====
        covs = self._batch_regularize_covariance(covs)

        # ===== 求逆与 logdet（批量）=====
        if low_rank and covs.size(-1) > max_rank:
            inv_covs, log_dets = self._batch_lowrank_inverse(covs, max_rank)
        else:
            inv_covs = torch.linalg.pinv(covs)
            log_dets = torch.logdet(covs)

        # ===== 保存参数 =====
        self.means = nn.Parameter(means.cpu(), requires_grad=False)
        self.regularized_covs = nn.Parameter(covs.cpu(), requires_grad=False)
        self.cov_invs = nn.Parameter(inv_covs.cpu(), requires_grad=False)
        self.log_dets = nn.Parameter(log_dets.unsqueeze(1).cpu(), requires_grad=False)

        # ===== 共享协方差 (LDA模式) =====
        if mode == "lda":
            shared_cov = covs.mean(dim=0)
            shared_cov = self._ensure_posdef(shared_cov)
            shared_inv = torch.linalg.pinv(shared_cov)
            shared_logdet = torch.logdet(shared_cov)
            self.shared_cov = nn.Parameter(shared_cov.cpu(), requires_grad=False)
            self.shared_cov_inv = nn.Parameter(shared_inv.cpu(), requires_grad=False)
            self.shared_log_det = nn.Parameter(shared_logdet.unsqueeze(0).cpu(), requires_grad=False)

    # ==========================================================
    # 正则化与低秩近似
    # ==========================================================
    def _batch_regularize_covariance(self, covs: torch.Tensor) -> torch.Tensor:
        """
        对每个协方差矩阵批量正则化
        """
        global_cov = covs.mean(dim=0)
        d = covs.size(-1)
        eye = torch.eye(d, device=covs.device)
        alpha = self.reg_alpha

        if self.reg_type == "shrinkage":
            covs = (1 - alpha) * covs + alpha * global_cov
        elif self.reg_type == "diagonal":
            diag_covs = torch.diagonal(covs, dim1=-2, dim2=-1)
            diag_mats = torch.diag_embed(diag_covs)
            covs = (1 - alpha) * covs + alpha * diag_mats
        elif self.reg_type == "spherical":
            traces = covs.diagonal(dim1=-2, dim2=-1).sum(dim=-1, keepdim=True)
            sphericals = (traces / d).unsqueeze(-1) * eye
            covs = (1 - alpha) * covs + alpha * sphericals
        # 其他模式默认不变

        covs = covs + 1e-5 * eye  # 数值稳定
        return covs

    def _batch_lowrank_inverse(self, covs: torch.Tensor, rank: int):
        """
        低秩近似批量求逆和 logdet
        """
        C, D, _ = covs.shape
        U, S, Vh = torch.linalg.svd(covs, full_matrices=False)
        ...
        U_r = U[:, :, :r]
        S_r = S[:, :r]
        inv_covs = (U_r * (1.0 / S_r).unsqueeze(1)) @ U_r.transpose(-1, -2)
        log_dets = S_r.log().sum(dim=1)
        return inv_covs, log_dets

    def _ensure_posdef(self, cov: torch.Tensor) -> torch.Tensor:
        """确保矩阵正定"""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        batch_size = x.size(0)
        num_classes, dim = self.means.shape
        logits = torch.zeros(batch_size, num_classes, device=x.device)

        if self.mode == "lda":
            for i, cid in enumerate(self.class_ids):
                logits[:, i] = self._lda_discriminant(x, i)
        else:
            for i, cid in enumerate(self.class_ids):
                logits[:, i] = self._qda_discriminant(x, i)
        return logits

    def _qda_discriminant(self, x: torch.Tensor, idx: int) -> torch.Tensor:
        mean = self.means[idx].to(x.device)
        cov_inv = self.cov_invs[idx].to(x.device)
        log_det = self.log_dets[idx].to(x.device)
        prior = torch.log(torch.tensor(self.priors[self.class_ids[idx]], device=x.device))

        x_centered = x - mean.unsqueeze(0)
        mahal = 0.5 * torch.sum(x_centered @ cov_inv * x_centered, dim=1)
        return -mahal - 0.5 * log_det + prior

    def _lda_discriminant(self, x: torch.Tensor, idx: int) -> torch.Tensor:
        mean = self.means[idx].to(x.device)
        cov_inv = self.shared_cov_inv.to(x.device)
        prior = torch.log(torch.tensor(self.priors[self.class_ids[idx]], device=x.device))
        term1 = x @ cov_inv @ mean
        term2 = 0.5 * mean @ cov_inv @ mean
    ...
    def refine_classifiers_from_variants(self, fc: nn.Module, epochs: int = 5, 
        ...
        for name, stats in self.variants.items():
            ...
            try:
                ...
                # LDC、SDC、SeqFT without Cov -> LDA（共享协方差）
