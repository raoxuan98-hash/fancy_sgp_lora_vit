...
class Drift_Compensator(object):
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        ...
        if self.compensate and task_id > 1:
            ...
            try:
                print("\n[DPCRR] Performing real DPCRR correction ...")
                P_task = W_current_only
                # === Step 3. 对历史类执行 CIP + 校正 ===
                ...
                if "DPCR" in self.variants and len(self.variants["DPCR"]) > 0:
                    ...
                    for cid, st in old_stats.items():
                        ...
                        # CIP 子空间
                        U_r = self._principal_subspace_from_cov(M2, energy=energy, r_cap=r_cap)
                        P_ic = P_task @ (U_r @ U_r.T)
                ...
                # === Step 4. 合并校正后的旧类 + 当前任务的新类 ===
                current_stats = self._build_stats(feats_after, labels_curr)
                merged_stats = copy.deepcopy(dpcrr_corrected_stats)
                merged_stats.update(copy.deepcopy(current_stats))

                # === Step 5. 存入新的变体 "DPCRR" ===
                if not hasattr(self, "variants"):
                    self.variants = {}
                self.variants["DPCRR"] = merged_stats

                print(f"[INFO] DPCRR variant built with {len(merged_stats)} classes "
                      f"(corrected old {len(dpcrr_corrected_stats)}, new {len(current_stats)})")

            except Exception as e:
                print(f"[ERROR] DPCRR compensation failed: {e}")
                print("[INFO] Fallback to original DPCR without correction.")
