...
class Drift_Compensator(object):
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        ...
        if self.compensate and task_id > 1:
            ...
            # ============ DPCR（类行空间收缩） ============
            try:
                print("\n[DPCR] Performing DPCR correction ...")
                
                # DPCR（仅当前任务）
                if W_current_only is not None:
                    W_dict_current = self._build_classwise_W(W_current_only, feats_before, labels)
                    if "DPCR" in self.variants:
                        stats_dpcr_compensated = self.transform_stats_with_W_classwise(
                            self.variants['DPCR'], W_dict_current)
                    else:
                        stats_dpcr_compensated = {}
                    self.variants["DPCR"] = stats_dpcr_compensated
                    self.variants["DPCR"].update(copy.deepcopy(stats))
                    print(f"[DPCR] Built with {len(stats_dpcr_compensated)} compensated + {len(stats)} new classes")

                # DPCR + ADE（使用ADE增强的W）
                if W is not None and aux_loader is not None:
                    # 使用合并后的特征构建类专属W
                    W_dict_ade = self._build_classwise_W(W, feats_before_combined, labels)
                    if "DPCR + ADE" in self.variants:
                        stats_dpcr_ade_compensated = self.transform_stats_with_W_classwise(
                            self.variants['DPCR + ADE'], W_dict_ade)
                    else:
                        stats_dpcr_ade_compensated = {}
                    self.variants["DPCR + ADE"] = stats_dpcr_ade_compensated
                    self.variants["DPCR + ADE"].update(copy.deepcopy(stats))
                    print(f"[DPCR+ADE] Built with {len(stats_dpcr_ade_compensated)} compensated + {len(stats)} new classes")

            except Exception as e:
                print(f"[ERROR] DPCR compensation failed: {e}")
                # 回退到普通补偿
                if "DPCR" in self.variants and W_current_only is not None:
                    stats_dpcr_fallback = self.transform_stats_with_W(self.variants['DPCR'], W_current_only)
                    self.variants["DPCR"] = stats_dpcr_fallback
                    self.variants["DPCR"].update(copy.deepcopy(stats))
                if "DPCR + ADE" in self.variants and W is not None:
                    stats_dpcr_ade_fallback = self.transform_stats_with_W(self.variants['DPCR + ADE'], W)
                    self.variants["DPCR + ADE"] = stats_dpcr_ade_fallback
                    self.variants["DPCR + ADE"].update(copy.deepcopy(stats))
