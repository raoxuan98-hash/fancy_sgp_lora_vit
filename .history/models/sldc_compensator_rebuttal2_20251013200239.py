...
class Drift_Compensator(object):
        ...
        for name, stats in self.variants.items():
            ...
            try:
                ...
                #     print(f"[INFO] {name}: QDA with {len(stats)} classes")
                    ...
                    out[f"{name} (LS)"] = ls_clf

                    print(f"[INFO] {name}: QDA/LS with {len(stats)} classes")
    ...
    def train_least_squares_classifier(self, stats: Dict[int, object], reg_lambda: float = 1e-3) -> nn.Module:
        ...
        W = torch.linalg.solve(X.T @ X + reg_I, X.T @ Y)  # [D, C]

        # --- 转为 nn.Linear ---
        fc = nn.Linear(d, num_classes, bias=False).to(device)
        fc.weight.data = W.T.clone()  # Linear expects [C, D]
