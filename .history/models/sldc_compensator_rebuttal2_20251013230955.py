...
class Drift_Compensator(object):
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        ...
        if self.compensate and task_id > 1:
            ...
            print(f"[INFO] DPCRR + ADE variant built with {len(merged_stats)} classes ")            
    ...
    def refine_classifiers_from_variants(self, fc: nn.Module, epochs: int = 5, 
                                        reg_alpha: float = 0.3, reg_type: str = "shrinkage") -> Dict[str, nn.Module]:
        """
        ...
        for name, stats in self.variants.items():
            ...
            # SeqFT without Cov -> LDA（共享协方差）
                ...
                print(f"[INFO] {name}: LS with {len(stats)} classes")
    ...
    def train_least_squares_classifier(self, stats: Dict[int, object], reg_lambda: float = 1e-3) -> nn.Module:
        ...
        W = torch.linalg.solve(X_normalized.T @ X_normalized + reg_I, X_normalized.T @ Y)  # [D, C]
        ...
        print(f"[INFO] LS-classifier built: {num_classes} classes, dim={d}, λ={reg_lambda}")
        print(f"[INFO] Features normalized to unit sphere, model structure: Normalize -> Linear")
        return classifier
