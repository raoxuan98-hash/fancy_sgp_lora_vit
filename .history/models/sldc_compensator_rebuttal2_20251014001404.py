...
class RegularizedGaussianClassifier(nn.Module):
    def __init__(self, stats_dict: Dict[int, 'GaussianStatistics'], 
        ...
        logging.info(f"[INFO] GaussianClassifier initialized on CPU with {self.num_classes} classes (mode={mode})")
    ...
    # 重写 cuda/cpu 方法以确保一致性
    def cuda(self, device=None):
        super().cuda(device)
        # 不立即移动参数，等到 forward 时根据输入设备决定
        self._current_device = torch.device("cpu")
        return self
