...
class WeakNonlinearTransform:
    ...
    def train(self, features_before: torch.Tensor, features_after: torch.Tensor,
        ...
        # ✅ 显式迁移到 GPU
        X = F.normalize(features_before.to(device), dim=-1)
        Y = F.normalize(features_after.to(device), dim=-1)

        for epoch in range(epochs):
            optimizer.zero_grad()
            idx = torch.randint(0, X.size(0), (64,), device=device)  # 与 X 同设备
            pred = self.net(X[idx])
...
class Drift_Compensator(object):
    ...
    def train_classifier_with_cached_samples(self, fc, stats, epochs=5):
        ...
        for cid, gauss in stats.items():
            ...
            X = m + Z @ L.t()
            y = torch.full((X.size(0),), int(cid), device=device, dtype=torch.long)
            samples_all.append(X)
            targets_all.append(y)

        X = torch.cat(samples_all, dim=0)
