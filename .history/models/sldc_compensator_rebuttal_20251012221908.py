...
class Drift_Compensator(object):
    ...
    def build_all_variants(self, task_id: int, model_before: nn.Module, model_after: nn.Module, data_loader):
        ...
        # —— LDC + ADE：用 α1-SLDC + ADE 的 W（含辅助数据）来投影历史原型，再合并当前任务原型 ——
        if task_id == 1:
            self.ldc_compensator_ADE = LDC_Compensator(self.args)
            self.ldc_compensator_ADE.update_task_with_W(task_id, feats_after, labels, W_prev_to_curr=None)
        else:
            self.ldc_compensator_ADE.update_task_with_W(task_id, feats_after, labels, W_prev_to_curr=W)
        ...
        for cid, proto in ldc_prototypes.items():
         ...
         # 新增 LDC+ADE 变体
        ldc_prototypes_ADE = self.ldc_compensator_ADE.get_prototypes()
        ldc_stats_ADE = {}
        for cid, proto in ldc_prototypes_ADE.items():
            d = proto.size(0)
            cov = torch.eye(d) * 1e-4
            ldc_stats_ADE[cid] = GaussianStatistics(proto, cov, reg=1e-5)
        ...
        self.variants["LDC + ADE"] = ldc_stats_ADE  # ✅ 新增
