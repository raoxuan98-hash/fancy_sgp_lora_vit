...
class Drift_Compensator(object):
    def __init__(self, args):
        ...
        # >>> 新增：解析分类器开关与正则
        self.use_analytic = args.get('use_analytic', True)
        self.ls_reg = args.get('ls_reg', 1e-4)
    ...
    def refine_classifiers_from_variants(self, fc: nn.Module, epochs: int = 5) -> Dict[str, nn.Module]:
        ...
        for name, stats in self.variants.items():
            if self.use_analytic:
                # 确保有缓存噪声
                if self.cached_Z is None:
                    # 与 build_all_variants 中保持一致的默认规模
                    first_stat = next(iter(stats.values()))
                    D = first_stat.mean.numel()
                    self.cached_Z = torch.randn(50000, D, device=self.device)

                C = len(stats)
                new_fc = nn.Linear(D, C, bias=True)
                new_fc = new_fc.to(device=self.device, dtype=fc.weight.dtype if fc is not None else torch.float32)
                clf = self.fit_classifier_with_least_squares(new_fc, stats,
                                                             num_samples_per_class=1024)
            else:
                clf = self.train_classifier_with_cached_samples(fc, stats, epochs=epochs)
            out[name] = clf
        print(f"[INFO] Built {len(out)} classifiers from variants using "
              f"{'analytic LS' if self.use_analytic else 'SGD training'}.")
        return out
