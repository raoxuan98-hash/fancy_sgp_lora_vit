...
class Drift_Compensator(object):
    ...
    def refine_classifiers_from_variants(self, fc: nn.Module, epochs: int = 5, use_qda: bool = False) -> Dict[str, nn.Module]:
        """
        对 self.variants 里的每个分布各训练一个分类器
        通过use_qda参数控制使用SGD训练还是QDA分类器
        """
        ...
        if use_qda:
            print("使用QDA分类器...")
            # 使用QDA分类器
            return self.build_qda_classifiers(self.variants)
        else:
            print("使用SGD训练的线性分类器...")
            # 使用SGD训练的线性分类器
            out = {}
...
if __name__ == "__main__":
    ...
    # 测试构建分类器 - 使用SGD
    classifiers_sgd = compensator.build_all_classifiers(
        task_id=1, 
        model_before=model_before, 
        model_after=model_after, 
        data_loader=data_loader,
        use_qda=False  # 使用SGD训练
    )
    
    # 测试构建分类器 - 使用QDA
    classifiers_qda = compensator.build_all_classifiers(
        task_id=1, 
        model_before=model_before, 
        model_after=model_after, 
        data_loader=data_loader,
    ...
    print("SGD分类器构建完成！")
    if classifiers_sgd['linear']:
        print(f"构建了 {len(classifiers_sgd['linear'])} 个SGD分类器")
    
    print("QDA分类器构建完成！")
    if classifiers_qda['qda']:
        print(f"构建了 {len(classifiers_qda['qda'])} 个QDA分类器")
