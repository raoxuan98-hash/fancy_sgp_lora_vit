...
class MahalanobisClassifier(nn.Module):
    """基于马氏距离的分类器"""
    def __init__(self, n_classes, feature_dim, use_cholesky=True, reg=1e-5):
    ...
    def fit(self, stats_dict, priors=None):
        ...
        for i, cid in enumerate(class_ids):
            ...
            if self.use_cholesky:
                # 使用Cholesky分解提高数值稳定性
                L = cholesky_manual_stable(reg_cov, reg=self.reg)
                self.cov_cholesky[i] = L
                # 计算对数行列式: log|Σ| = 2 * sum(log(diag(L)))
                self.log_dets[i] = 2 * torch.log(torch.diag(L)).sum()
            else:
                # 直接计算行列式
                self.log_dets[i] = torch.logdet(reg_cov)
    ...
    def mahalanobis_distance(self, x, class_idx):
        ...
        if self.use_cholesky:
            # 使用Cholesky分解高效求解: Σ^{-1}(x-μ) = L^{-T} L^{-1}(x-μ)
    ...
    def log_probability(self, x):
        ...
        for i in range(self.n_classes):
            ...
            # 高斯分布的对数概率密度（忽略常数项）
            # log p(x|class_i) = -0.5 * [mahalanobis + log|Σ_i| + D*log(2π)]
            # 由于常数项对所有类相同，可以省略
            log_likelihood = -0.5 * (mahalanobis + self.log_dets[i])
            
            # 加上先验概率
            log_probs[:, i] = log_likelihood + torch.log(self.priors[i])
    ...
    def forward(self, x):
        ...
        if not self.is_fitted:
            raise ValueError("马氏距离分类器尚未拟合数据")
    ...
    def distance_based_confidence(self, x):
        """基于距离的置信度分数"""
        log_probs = self.forward(x)
    ...
    def compute_batch_distances(self, x):
        """批量计算到所有类的马氏距离"""
        batch_size = x.size(0)
        distances = torch.zeros(batch_size, self.n_classes, device=x.device)
        
        for i in range(self.n_classes):
            distances[:, i] = self.mahalanobis_distance(x, i)
...
class FastMahalanobisClassifier(nn.Module):
    """快速马氏距离分类器 - 使用共享协方差矩阵"""
    def __init__(self, n_classes, feature_dim, use_cholesky=True, reg=1e-5):
        super(FastMahalanobisClassifier, self).__init__()
        self.n_classes = n_classes
        self.feature_dim = feature_dim
        self.use_cholesky = use_cholesky
        self.reg = reg
        ...
        # 存储参数
        self.register_buffer('means', torch.zeros(n_classes, feature_dim))
        self.register_buffer('shared_cov', torch.eye(feature_dim))
        self.register_buffer('shared_cov_cholesky', torch.eye(feature_dim))
        self.register_buffer('shared_log_det', torch.tensor(0.0))
        self.register_buffer('priors', torch.zeros(n_classes))
    ...
    def fit(self, stats_dict, priors=None):
        ...
        # 计算共享协方差矩阵（所有类的平均）
        shared_cov = torch.zeros(self.feature_dim, self.feature_dim)
        for cid, stat in stats_dict.items():
            shared_cov = shared_cov + stat.cov
        shared_cov = shared_cov / len(stats_dict)
        
        # 正则化
        shared_cov = shared_cov + self.reg * torch.eye(self.feature_dim, device=shared_cov.device)
        self.shared_cov = shared_cov
        
        if self.use_cholesky:
            L = cholesky_manual_stable(shared_cov, reg=self.reg)
            self.shared_cov_cholesky = L
            self.shared_log_det = 2 * torch.log(torch.diag(L)).sum()
        else:
            self.shared_log_det = torch.logdet(shared_cov)
    ...
    def mahalanobis_distance(self, x, class_idx):
        ...
        if self.use_cholesky:
            L = self.shared_cov_cholesky
            z = torch.linalg.solve_triangular(L, diff.T, upper=False)
            distance = (z ** 2).sum(dim=0)
        else:
            cov_inv = torch.linalg.inv(self.shared_cov)
            distance = (diff @ cov_inv @ diff.T).diag()
    ...
    def forward(self, x):
        """前向传播"""
        if not self.is_fitted:
            raise ValueError("快速马氏距离分类器尚未拟合")
        ...
        for i in range(self.n_classes):
            mahalanobis = self.mahalanobis_distance(x, i)
            log_probs[:, i] = -0.5 * (mahalanobis + self.shared_log_det) + torch.log(self.priors[i])
