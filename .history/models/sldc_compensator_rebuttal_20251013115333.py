...
# 首先定义 GaussianStatistics 类（根据您的原始代码假设）
class RegularizedGaussianClassifier(nn.Module):
    def __init__(self, stats_dict: Dict[int, GaussianStatistics], 
        ...
        for cid in self.class_ids:
            ...
            regularized_cov = self._regularize_covariance(original_cov)
        ...
        if mode == "lda":
            regularized_shared_cov = self._regularize_covariance(self.global_cov)
    ...
    def _regularize_covariance(self, cov: torch.Tensor) -> torch.Tensor:
        ...
        if self.reg_type == "shrinkage":
            ...
            target = (1 - 0.5) * self.global_cov + 0.5 * diag_global  # 可以调整权重
        ...
        # 确保正定性
        regularized = self._ensure_positive_definite(regularized)
        return regularized
    ...
    def _lda_discriminant(self, x: torch.Tensor, class_id: int) -> torch.Tensor:
        ...
        term1 = x @ self.shared_cov_inv @ mean
        term2 = 0.5 * mean @ self.shared_cov_inv @ mean
    ...
    def refine_classifiers_from_variants(self, fc: nn.Module, epochs: int = 5, method: str = "qda", 
        ...
        Args:
            ...
            method: 
                ...
                "qda" - 二次判别分析分类器
                "lda" - 线性判别分析分类器
            reg_alpha: 正则化强度 (0-1之间)，对qda/lda有效
            reg_type: 正则化类型 ("shrinkage", "diagonal", "combined", "spherical")，对qda/lda有效
