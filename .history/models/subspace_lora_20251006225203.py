...
class SubspaceLoRA(BaseLearner):
    ...
    def process_batch(
        ...
        """
        优化版：仅当 gamma_kd>0 时才使用 teacher 网络。
        """
        ...
        if aux_inputs is not None:
            ...
            feats, feats_aux = feats_concat.split([b_main, aux_inputs.size(0)], dim=0)
        else:
            feats = feats_concat
            feats_aux = None

        # === Teacher 网络：只有在 KD 开启时才 forward ===
        if self.use_feature_kd:
            with torch.no_grad():
                if self.use_aux_for_kd and aux_inputs is not None:
                    prev_feats_aux = self.prev_network.forward_features(aux_inputs)
                    prev_feats = None
                else:
                    prev_feats = self.prev_network.forward_features(inputs)
                    prev_feats_aux = None
