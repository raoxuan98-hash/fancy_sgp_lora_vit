...
class SubspaceLoRA(BaseLearner):
    ...
    def incremental_train(self, data_manager) -> None:
        ...
        # === 初始化当前任务的蒸馏头 ===
        feat_dim = self.network.vit.embed_dim
        self.distill_head = nn.Linear(feat_dim, feat_dim, bias=False).to(self._device)
        nn.init.orthogonal_(self.distill_head.weight)
    ...
    def make_optimizer(
        ...
        param_groups = [
            ...
            {"params": distill_params, "lr": 1e-3 if self.optimizer_type == "adamw" else 5e-3, "weight_decay": 0.0},
        ]
    ...
    def process_batch(
        ...
        #     kd_feat = self.kd_loss_fn(teacher_transformed, feats)
        #     kd_norm = self.norm_loss(teacher_transformed, feats)
        ...
        if self.use_feature_kd:
            ...
            student_norm = torch.nn.functional.normalize(feats, dim=-1)
            teacher_norm = torch.nn.functional.normalize(teacher_transformed, dim=-1)
