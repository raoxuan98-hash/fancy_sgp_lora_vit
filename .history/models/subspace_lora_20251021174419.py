...
class SubspaceLoRA(BaseLearner):
    def __init__(self, args: Dict[str, Any]) -> None:
        ...
        self.gamma_kd: float = args["gamma_kd"] if self.use_feature_kd else 1.0
        ...
        self.loss_smoother = EMASmooth(alpha=0.95)
        self.kd_smoother = EMASmooth(alpha=0.95)
        self.acc_smoother = EMASmooth(alpha=0.95)
    ...
    def incremental_train(self, data_manager) -> None:
        ...
        max_samples = getattr(self, 'max_train_test_samples', 100000)
        sampler = None
    ...
    def count_trainable_parameters(self) -> Dict[str, int]:
        ...
        if self.distill_head is not None:
            distill_count = sum(p.numel() for p in self.distill_head.parameters())
