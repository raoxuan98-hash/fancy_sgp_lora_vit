...
class SubspaceLoRA(BaseLearner):
    ...
    def incremental_train(self, data_manager) -> None:
        ...
        try:
            ...
            logging.info(
                "System training on classes %d-%d",
                self._known_classes,
                self._total_classes,
                data_manager.dataset_names[t].lower())
    ...
    def evaluate(
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                if batch_idx == 0:
                    debug_batch = {
                        ...
                        'targets_range': (targets.min().item(), targets.max().item()),
                        'targets_unique': targets.unique().tolist()
                    }
                    ...
                    for name, fc in fc_dict.items():
                        feats = self.network.forward_features(inputs)
                        logits = fc(feats)
                        debug_batch[f'{name}_logits_shape'] = logits.shape
                        preds = logits.argmax(dim=1).cpu()
                        debug_batch[f'{name}_preds_range'] = (preds.min().item(), preds.max().item())
                        debug_batch[f'{name}_preds_unique'] = preds.unique().tolist()
        ...
        if debug_batch is not None:
            logging.info(f"[EVAL] Debug info for first batch:")
            logging.info(f"[EVAL]   Input shape: {debug_batch['inputs_shape']}")
            logging.info(f"[EVAL]   Targets range: {debug_batch['targets_range']}")
            logging.info(f"[EVAL]   Targets unique (first 10): {debug_batch['targets_unique'][:10]}")
            
            for name, fc in fc_dict.items():
                logits_shape = debug_batch[f'{name}_logits_shape']
                preds_range = debug_batch[f'{name}_preds_range']
                preds_unique = debug_batch[f'{name}_preds_unique']
                
                logging.info(f"[EVAL]   {name}:")
                logging.info(f"[EVAL]     Logits shape: {logits_shape}")
                logging.info(f"[EVAL]     Preds range: {preds_range}")
                logging.info(f"[EVAL]     Preds unique (first 10): {preds_unique[:10]}")
                
                # 检查预测是否超出目标范围
                if preds_range[1] > debug_batch['targets_range'][1]:
                    logging.warning(f"[EVAL] WARNING: {name} preds max {preds_range[1]} > targets max {debug_batch['targets_range'][1]}")
