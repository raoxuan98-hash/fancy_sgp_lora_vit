...
class SubspaceLoRA(BaseLearner):
    ...
    def process_batch(self, inputs, targets, optimizer):
        ...
        # 在跨域非累积模式下，标签已经是局部索引（从0开始）
        # 不需要再减去self._known_classes
        if self.args.get('cross_domain', False):
            # 跨域场景：直接使用局部标签
            new_targets_rel = targets
            new_logits = logits
            
            # 调试：验证logits维度与标签范围的匹配
            if not hasattr(self, '_logits_debug_logged') and self.current_task_id == 0:
                logging.info(f"[TRAIN] Cross-domain mode debug:")
                logging.info(f"[TRAIN]   Logits shape: {logits.shape}")
                logging.info(f"[TRAIN]   Targets shape: {targets.shape}")
                logging.info(f"[TRAIN]   Expected num classes: {self._total_classes - self._known_classes}")
    ...
    def _evaluate_cross_domain(self, loader: DataLoader, fc_dict):
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                # Determine which task each sample belongs to based on label ranges
        ...
        for name in fc_dict.keys():
            ...
            for task_id in range(num_tasks):
                if task_totals[task_id] > 0:
                    ...
                    task_accuracies[name].append(0.0)
        
        # Log per-task accuracies for debugging
        logging.info("Cross-domain per-task accuracies:")
        for name in fc_dict.keys():
            logging.info(f"  Variant: {name}")
            for task_id, acc in enumerate(task_accuracies[name]):
                # 获取数据集名称
                dataset_name = data_manager.dataset_names[task_id]
                task_status = "LEARNED" if task_id < self.current_task_id else "NOT LEARNED"
                logging.info(f"    {dataset_name} (task {task_id}, {task_status}): {acc:.2f}%")
            
            # DEBUG: 分别计算已学习任务和所有任务的平均准确度
            learned_tasks_acc = [task_accuracies[name][i] for i in range(self.current_task_id)]
            all_tasks_acc = task_accuracies[name]
            
            avg_learned = np.mean(learned_tasks_acc) if learned_tasks_acc else 0.0
            avg_all = np.mean(all_tasks_acc)
            
            logging.info(f"    Average (LEARNED tasks only): {avg_learned:.2f}%")
            logging.info(f"    Average (ALL tasks): {avg_all:.2f}%")
        
        # Return average across tasks (each task weighted equally)
        results = {}
        for name in fc_dict.keys():
            # DEBUG: 目前返回所有任务的平均值（这是问题所在）
            all_tasks_acc = task_accuracies[name]
            results[name] = float(np.around(np.mean(all_tasks_acc), 2))
            
            # DEBUG: 记录应该返回的值（只计算已学习任务）
            learned_tasks_acc = [task_accuracies[name][i] for i in range(self.current_task_id)]
            should_return = float(np.around(np.mean(learned_tasks_acc), 2)) if learned_tasks_acc else 0.0
            
            logging.info(f"[DEBUG] Variant {name}:")
            logging.info(f"[DEBUG]   Currently returns (all tasks): {results[name]}")
            logging.info(f"[DEBUG]   Should return (learned tasks only): {should_return}")
