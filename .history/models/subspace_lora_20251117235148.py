...
class SubspaceLoRA(BaseLearner):
    ...
    def process_batch(self, inputs, targets, optimizer):
        ...
        # 在跨域场景下，标签是全局的（从0开始的累积标签）
        # 需要转换为局部索引（相对于当前任务开始位置）
        if self.args.get('cross_domain', False):
            # 跨域场景：将全局标签转换为局部标签
            new_targets_rel = targets - self._known_classes
            ...
            if not hasattr(self, '_logits_debug_logged') and self.current_task_id == 0:
                ...
                logging.info(f"[TRAIN]   Global label range: {targets.min().item()}-{targets.max().item()}")
                logging.info(f"[TRAIN]   Local label range: {new_targets_rel.min().item()}-{new_targets_rel.max().item()}")
                logging.info(f"[TRAIN]   _known_classes: {self._known_classes}")
                logging.info(f"[TRAIN]   _total_classes: {self._total_classes}")
