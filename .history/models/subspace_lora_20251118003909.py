...
class SubspaceLoRA(BaseLearner):
    ...
    def _evaluate_cross_domain(self, loader: DataLoader, fc_dict):
        ...
        logging.info(f"[DEBUG]   Should evaluate tasks: 1 to {self.current_task_id}")

        # Initialize dictionaries to track correct predictions and totals per class for each classifier
        class_corrects = {name: {} for name in fc_dict.keys()}
        class_totals = {}
        
        # Initialize all possible classes that might appear in evaluation
        # This should include all classes seen so far (from task 1 to current_task_id)
        expected_classes = set()
        for task_id in range(1, self.current_task_id + 1):
            task_classes = data_manager.get_task_size(task_id)
            start_class = data_manager.get_accumulated_task_size(task_id - 1) if task_id > 1 else 0
            expected_classes.update(range(start_class, start_class + task_classes))
        
        # Initialize dictionaries with all expected classes
        for class_id in expected_classes:
            class_totals[class_id] = 0
            for name in fc_dict.keys():
                class_corrects[name][class_id] = 0
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                for name, fc in fc_dict.items():
                    ...
                    # Count correct predictions and totals per class
                    for pred, target in zip(preds, targets):
                        target_item = target.item()
                        # Only count if this class is in our expected set
                        if target_item in class_totals:
                            class_totals[target_item] += 1
                            if pred.item() == target_item:
                                class_corrects[name][target_item] += 1
                        else:
                            logging.warning(f"[DEBUG] Unexpected class {target_item} encountered in evaluation")
        
        # Calculate class-wise accuracy for each classifier
        class_accuracies = {name: [] for name in fc_dict.keys()}
        
        for name in fc_dict.keys():
            for class_id in sorted(class_totals.keys()):
                if class_totals[class_id] > 0:
                    accuracy = class_corrects[name][class_id] / class_totals[class_id]
                    class_accuracies[name].append(accuracy)
            
            # Log class-wise accuracies for debugging
            if class_accuracies[name]:
                logging.info(f"[DEBUG] Class-wise accuracies for {name}:")
                for class_id in sorted(class_totals.keys()):
                    if class_totals[class_id] > 0:
                        acc = class_corrects[name][class_id] / class_totals[class_id]
                        logging.info(f"[DEBUG]   Class {class_id}: {acc:.4f} ({class_corrects[name][class_id]}/{class_totals[class_id]})")
        
        # Return average across all classes for each classifier
        results = {}
        for name in fc_dict.keys():
            if class_accuracies[name]:
                avg_accuracy = np.mean(class_accuracies[name])
                results[name] = float(np.around(avg_accuracy * 100, 3))  # Convert to percentage
                logging.info(f"[DEBUG] {name} average class-wise accuracy: {results[name]:.2f}% "
                            f"(across {len(class_accuracies[name])} classes)")
            else:
                results[name] = 0.0
                logging.warning(f"[DEBUG] No class-wise accuracy computed for {name}")
