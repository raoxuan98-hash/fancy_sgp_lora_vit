...
class SubspaceLoRA(BaseLearner):
    ...
    def _evaluate_cross_domain(self, loader: DataLoader, fc_dict):
        ...
        for task_id in range(self.current_task_id):
            ...
            task_size = data_manager.get_task_size(task_id)
            for class_idx in range(task_start, task_start + task_size):
                ...
                for task_id in range(self.current_task_id):
                    ...
                    if task_samples > 0:
                        ...
                        for name, fc in fc_dict.items():
                            ...
                            # 统计每个类别的准确率
                            for class_label in range(task_start, task_end):
                                class_mask = (task_targets == class_label)
        ...
        for name in fc_dict.keys():
            ...
            for class_label in class_totals:
        ...
        for name in fc_dict.keys():
            if class_accuracies[name]:
                # 计算每个数据集的平均准确率（用于调试信息）
                dataset_accuracies = {}
                for task_id in range(self.current_task_id):
                    task_start = data_manager.global_label_offset[task_id]
                    task_size = data_manager.get_task_size(task_id)
                    task_classes = list(range(task_start, task_start + task_size))
                    task_class_accs = [class_accuracies[name][i] for i in range(len(class_accuracies[name])) 
                                    if list(class_totals.keys())[i] in task_classes]
                    if task_class_accs:
                        dataset_acc = np.mean(task_class_accs)
                ...
                logging.info(f"    Class-wise Average: {all_class_acc:.2f}%")
