...
class SubspaceLoRA(BaseLearner):
    ...
    def _evaluate_cross_domain(self, loader: DataLoader, fc_dict):
        """
        Cross-domain evaluation: compute class-wise average accuracy.
        This ensures each class contributes equally to the final metric, regardless of dataset size.
        Only considers classes that have been learned so far (0 to self._total_classes-1).
        """
        self.network.eval()
        ...
        logging.info(f"[DEBUG]   Known classes: {self._known_classes}")
        logging.info(f"[DEBUG]   Total classes: {self._total_classes}")
        logging.info(f"[DEBUG]   Classes to evaluate: 0 to {self._total_classes - 1}")
        
        # Initialize per-class accuracy tracking for learned classes only
        class_corrects = {name: {} for name in fc_dict.keys()}
        class_totals = {}
        
        # 只初始化已经学过的类别（0 到 self._total_classes-1）
        for class_idx in range(self._total_classes):
            class_totals[class_idx] = 0
            for name in fc_dict.keys():
                class_corrects[name][class_idx] = 0
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if len(valid_inputs) > 0:
                    feats = self.network.forward_features(valid_inputs)
                    for name, fc in fc_dict.items():
                        preds = fc(feats).argmax(dim=1).cpu()
                        
                        # 统计每个已学类别的准确率
                        for class_label in range(self._total_classes):
                            class_mask = (valid_targets == class_label)
                            class_samples = class_mask.sum().item()
                            if class_samples > 0:
                                class_preds = preds[class_mask]
                                class_targets = valid_targets[class_mask]
                                class_corrects[name][class_label] += (class_preds == class_targets).sum().item()
                                class_totals[class_label] += class_samples
        
        # Compute per-class accuracies for learned classes only
        class_accuracies = {}
        valid_classes = []  # 记录有样本的类别
        
        for name in fc_dict.keys():
            class_accuracies[name] = []
            for class_label in range(self._total_classes):
                if class_totals[class_label] > 0:
                    acc = 100 * class_corrects[name][class_label] / class_totals[class_label]
                    class_accuracies[name].append(float(np.around(acc, 2)))
                    if name == list(fc_dict.keys())[0]:  # 只在第一个variant时记录
                        valid_classes.append(class_label)
        
        # Log per-class accuracies summary
        logging.info("Cross-domain class-wise accuracies summary:")
        logging.info(f"Evaluated {len(valid_classes)} learned classes: {valid_classes}")
        ...
        for name in fc_dict.keys():
            if class_accuracies[name]:
                ...
                for task_id in range(self.current_task_id):
                    ...
                    task_size = data_manager.get_task_size(task_id)
                    task_classes = list(range(task_start, task_start + task_size))
                    
                    # 只考虑在当前任务中且已学习的类别
                    current_task_classes = [c for c in task_classes if c < self._total_classes]
                    task_class_accs = [class_accuracies[name][valid_classes.index(c)] for c in current_task_classes 
                                    if c in valid_classes]
                    
                    if task_class_accs:
                        task_acc = np.mean(task_class_accs)
                        dataset_name = data_manager.dataset_names[task_id]
                        task_accuracies[dataset_name] = task_acc
                
                # 计算所有已学类别的平均准确率
                all_class_acc = np.mean(class_accuracies[name])
                
                logging.info(f"  Variant: {name}")
                for dataset_name, acc in task_accuracies.items():
                    logging.info(f"    {dataset_name}: {acc:.2f}%")
                logging.info(f"    Class-wise Average ({len(class_accuracies[name])} classes): {all_class_acc:.2f}%")
        
        # Return class-wise average across all learned classes
        results = {}
        for name in fc_dict.keys():
            if class_accuracies[name]:
