...
class SubspaceLoRA(BaseLearner):
    ...
    def _evaluate_cross_domain(self, loader: DataLoader, fc_dict):
        ...
        Also computes class-wise average accuracy for cross-domain scenarios.
        """
        self.network.eval()
        ...
        # 计算并打印每个类别的准确度（跨域场景）
        logging.info("Cross-domain per-class accuracies:")
            ...
            for task_id in range(self.current_task_id):
                ...
                for class_id in range(task_start, task_end):
                    if class_id in per_class_totals[name] and per_class_totals[name][class_id] > 0:
                        ...
                        logging.info(f"      Class {class_id}: {acc:.2f}% ({per_class_corrects[name][class_id]}/{per_class_totals[name][class_id]})")
                    else:
                        logging.info(f"      Class {class_id}: 0.00% (0/0)")
            
            # 计算平均类别准确度
            if class_accuracies:
                avg_class_acc = np.mean(class_accuracies)
                class_wise_accuracies[name] = float(np.around(avg_class_acc, 2))
                logging.info(f"    Average per-class accuracy: {avg_class_acc:.2f}%")
            else:
