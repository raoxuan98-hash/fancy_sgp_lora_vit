...
class SubspaceLoRA(BaseLearner):
    ...
    def _evaluate_cross_domain(self, loader: DataLoader, fc_dict):
        ...
        # 添加class-wise平均准确度到返回结果
        results["class_wise_accuracies"] = class_wise_accuracies
        # 添加每个任务的class-wise平均准确度
        results["per_task_class_wise_accuracies"] = per_task_class_wise_accuracies
    ...
    def eval_task(self):
        ...
        if is_cross_domain and self.current_task_id > 0:
            ...
            # 如果有class-wise准确度信息，也记录
            if 'per_task_class_wise_accuracies' in results:
                logging.info(f"📊 任务 {self.current_task_id} 完成后的各数据集Class-wise准确度:")
                for variant, class_wise_accs in results['per_task_class_wise_accuracies'].items():
                    if isinstance(class_wise_accs, list) and len(class_wise_accs) > 0:
                        # 获取当前任务的class-wise准确度（最后一个值）
                        current_task_class_wise_acc = class_wise_accs[-1]
                        logging.info(f"  {variant:<20}: {current_task_class_wise_acc:.2f}%")
    ...
    def analyze_task_results(self, all_task_results: Dict[int, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if is_cross_domain and 'class_wise_accuracies' in all_task_results[last_task_id]:
            class_wise_accuracies = all_task_results[last_task_id]['class_wise_accuracies']
        ...
        if is_cross_domain:
            ...
            for variant in variant_names:
                ...
                for task_id in task_ids:
                    if 'per_task_class_wise_accuracies' in all_task_results[task_id]:
                        per_task_accs = all_task_results[task_id]['per_task_class_wise_accuracies']
