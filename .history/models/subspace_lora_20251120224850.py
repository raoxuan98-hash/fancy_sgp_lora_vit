            ...
            for batch_idx, batch in enumerate(loader):
                ...
                for name, fc in fc_dict.items():
                    ...
                    self._update_task_statistics(
                        name, preds, targets, task_stats[name], 
                        data_manager, batch_idx
                    )
        
        return self._compute_final_results(task_stats, fc_dict, data_manager)

    def _update_task_statistics(self, name, preds, targets, stats, data_manager, batch_idx):
        """更新任务统计信息"""
        # 只评估已学习的任务
        for task_id in range(self.current_task_id):
            ...
            if task_samples > 0:
                ...
                # 更新任务级别统计
                stats['task_corrects'][task_id] += (task_preds == task_targets).sum().item()
                stats['task_totals'][task_id] += task_samples
                
                # 更新类别级别统计
                self._update_class_statistics(
                    name, task_preds, task_targets, task_start, task_end, 
                    stats, batch_idx
                )

    def _update_class_statistics(self, name, preds, targets, task_start, task_end, stats, batch_idx):
        """更新类别级别的统计信息"""
        for class_id in range(task_start, task_end):
            ...
            if class_total > 0:
                ...
                # 初始化或更新类别统计
                if class_id not in stats['per_class_corrects']:
                    stats['per_class_corrects'][class_id] = 0
                    stats['per_class_totals'][class_id] = 0
                
                stats['per_class_corrects'][class_id] += class_correct
                stats['per_class_totals'][class_id] += class_total

    def _compute_final_results(self, task_stats, fc_dict, data_manager):
        """计算最终评估结果"""
        results = {}
        class_wise_data = {
        ...
        for name in fc_dict.keys():
            ...
            # 计算任务准确度
            task_accuracies = self._compute_task_accuracies(stats)
            results[name] = self._compute_average_accuracy(task_accuracies)
            
            # 计算类别准确度
            class_wise_results = self._compute_class_wise_accuracies(stats, data_manager)
            class_wise_data["class_wise_accuracies"][name] = class_wise_results["overall"]
            class_wise_data["per_task_class_wise_accuracies"][name] = class_wise_results["per_task"]
        
        # 记录详细结果
        self._log_detailed_results(results, class_wise_data, data_manager)
        
        # 将类别准确度数据附加到结果中
        results["_class_wise_data"] = class_wise_data
        
        return results

    def _compute_task_accuracies(self, stats):
        """计算每个任务的准确度"""
        task_accuracies = []
        for task_id in range(self.current_task_id):
            if stats['task_totals'][task_id] > 0:
                acc = 100 * stats['task_corrects'][task_id] / stats['task_totals'][task_id]
                task_accuracies.append(float(np.around(acc, 2)))
        return task_accuracies

    def _compute_average_accuracy(self, task_accuracies):
        """计算平均准确度"""
        if task_accuracies:
            return float(np.around(np.mean(task_accuracies), 2))
        else:
            return 0.0

    def _compute_class_wise_accuracies(self, stats, data_manager):
        """计算类别级别的准确度"""
        overall_class_accuracies = []
        per_task_class_accuracies = []
        ...
        for task_id in range(self.current_task_id):
            ...
            for class_id in range(task_start, task_end):
                if (class_id in stats['per_class_totals'] and 
                    stats['per_class_totals'][class_id] > 0):
                    
                    acc = 100 * (stats['per_class_corrects'][class_id] / 
                            stats['per_class_totals'][class_id])
                    acc_value = float(np.around(acc, 2))
                    
                    overall_class_accuracies.append(acc_value)
                    task_class_accuracies.append(acc_value)
            
            # 计算当前任务的类别平均准确度
            if task_class_accuracies:
                task_avg_acc = np.mean(task_class_accuracies)
                per_task_class_accuracies.append(float(np.around(task_avg_acc, 2)))
            else:
                per_task_class_accuracies.append(0.0)
        
        # 计算总体类别平均准确度
        overall_avg_acc = np.mean(overall_class_accuracies) if overall_class_accuracies else 0.0
        
        return {
            "overall": float(np.around(overall_avg_acc, 2)),
            "per_task": per_task_class_accuracies
        }

    def _log_detailed_results(self, results, class_wise_data, data_manager):
        """记录详细的评估结果"""
        logging.info("📊 跨域评估详细结果:")
        
        # 记录每个变体的任务平均准确度
        logging.info("   ── 任务平均准确度 (%) ──")
        for name, accuracy in results.items():
            if isinstance(accuracy, (int, float)):
                logging.info(f"      {name:<20}: {accuracy:.2f}%")
        
        # 记录类别平均准确度
        logging.info("   ── 类别平均准确度 (%) ──")
        for name in results.keys():
            if name in class_wise_data["class_wise_accuracies"]:
                acc = class_wise_data["class_wise_accuracies"][name]
                logging.info(f"      {name:<20}: {acc:.2f}%")
        
        # 记录每个任务的准确度（如果可用）
        if hasattr(data_manager, 'dataset_names'):
            logging.info("   ── 各数据集准确度 (%) ──")
            for task_id in range(self.current_task_id):
                dataset_name = data_manager.dataset_names[task_id]
                task_accuracies = []
                for name in results.keys():
                    if (name in class_wise_data["per_task_class_wise_accuracies"] and
                        task_id < len(class_wise_data["per_task_class_wise_accuracies"][name])):
                        acc = class_wise_data["per_task_class_wise_accuracies"][name][task_id]
                        task_accuracies.append(acc)
                
                if task_accuracies:
                    avg_acc = np.mean(task_accuracies)
                    logging.info(f"      {dataset_name:<20}: {avg_acc:.2f}%")
