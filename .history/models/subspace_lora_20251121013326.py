...
class SubspaceLoRA(BaseLearner):
    ...
    def eval_task(self):
        ...
        if not hasattr(self, "all_task_results"):
            ...
            logging.info(f"[DEBUG] Initialized all_task_results as empty dict")

        # 添加日志来跟踪数据类型和内容
        logging.info(f"[DEBUG] Task {self.current_task_id} results type: {type(results)}")
        logging.info(f"[DEBUG] Task {self.current_task_id} results keys: {list(results.keys()) if isinstance(results, dict) else 'Not a dict'}")
        
        # 检查是否为跨域场景
        is_cross_domain = self.args['cross_domain']
        logging.info(f"[DEBUG] Cross-domain scenario: {is_cross_domain}")
        
        # 如果是字典，检查值的类型
        if isinstance(results, dict):
            for key, value in results.items():
                logging.info(f"[DEBUG]   {key}: {type(value)} (sample: {str(value)[:100]}...)" if isinstance(value, (dict, list)) else f"[DEBUG]   {key}: {type(value)} = {value}")
        
        self.all_task_results[self.current_task_id] = results
        logging.info(f"[DEBUG] Stored results for task {self.current_task_id} in all_task_results")
    ...
    def analyze_task_results(self, all_task_results, dataset_names):
        logging.info(f"[DEBUG] analyze_task_results called with all_task_results type: {type(all_task_results)}")
        logging.info(f"[DEBUG] all_task_results keys: {list(all_task_results.keys()) if all_task_results else 'Empty'}")
        ...
        logging.info(f"[DEBUG] Last task ID: {last_task_id}")

        # 检查每个任务的结果结构
        for task_id, task_result in all_task_results.items():
            logging.info(f"[DEBUG] Task {task_id} result type: {type(task_result)}")
            if isinstance(task_result, dict):
                logging.info(f"[DEBUG] Task {task_id} result keys: {list(task_result.keys())}")
                for key, value in task_result.items():
                    logging.info(f"[DEBUG]   {key}: {type(value)}")
        ...
        logging.info(f"[DEBUG] Found variant names: {variant_names}")
        ...
        # === 根据数据集类型输出不同的评估指标 ===
        if is_cross_domain:
            # For cross-domain datasets: output average_accuracy, task-wise average_accuracy, and class-wise accuracy
            logging.info("📊 Cross-domain Evaluation Results:")
            logging.info("   ── Current Task Average Accuracy (%) ──")
            for variant in variant_names:
                current_acc = last_task_accuracies[variant]
                logging.info(f"      {variant:<20} : {current_acc:.2f}%")
            
            # Extract task-wise accuracies from the last task results
            ...
            if 'average_task_wise_accs' in all_task_results[last_task_id]:
                ...
                for variant in variant_names:
                    if variant in task_wise_data:
                        ...
                        if task_accs:
                            task_wise_accuracies[variant] = float(np.mean(task_accs))
                            logging.info(f"      {variant:<20} task-wise avg: {task_wise_accuracies[variant]:.2f}%")
            
            # Extract class-wise accuracies from the last task results
            class_wise_accuracies = {}
            if 'average_class_wise_accs' in all_task_results[last_task_id]:
                ...
                for variant in variant_names:
                    if variant in class_wise_data:
                        class_wise_accuracies[variant] = class_wise_data[variant]
                        logging.info(f"      {variant:<20} class-wise avg: {class_wise_accuracies[variant]:.2f}%")
            
            # Also log per-task details
            if 'average_task_wise_accs' in all_task_results[last_task_id] and dataset_names:
                logging.info("   ── Per-Task Accuracy Details ──")
                task_wise_data = all_task_results[last_task_id]['average_task_wise_accs']
                for variant in variant_names:
                    if variant in task_wise_data:
            ...
            return {
                ...
                "task_wise_accuracies": task_wise_accuracies,
