...
class SubspaceLoRA(BaseLearner):
    ...
    def analyze_task_results(self, all_task_results, dataset_names):
        ...
        # 修正：从 average_accs 中提取 variant 名称
        variant_names = set()
        ...
        # 修正：从 average_accs 获取最终任务准确率
        last_task_accuracies = {}
        if 'average_accs' in all_task_results[last_task_id]:
            last_task_accuracies = all_task_results[last_task_id]['average_accs'].copy()
        ...
        if is_cross_domain:
            ...
            for variant in variant_names:
                ...
                logging.info(f"      {variant:<30} : {current_acc:.2f}%")
                ...
                for variant in variant_names:
                    if variant in task_wise_data:
                        ...
                        if task_accs:
                            ...
                            logging.info(f"      {variant:<30} task-wise avg: {task_wise_accuracies[variant]:.2f}%")
            ...
            if 'average_class_wise_accs' in all_task_results[last_task_id]:
                ...
                for variant in variant_names:
                    if variant in class_wise_data:
                        ...
                        logging.info(f"      {variant:<30} class-wise avg: {class_wise_accuracies[variant]:.2f}%")
