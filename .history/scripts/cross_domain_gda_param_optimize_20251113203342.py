...
def _prepare_args(template: Mapping, dataset: str, seed: int) -> Dict:
...
def optimise_dataset(
    ...
    args = _prepare_args(base_args, dataset, seed)
    ...
    if not retain_features:
        features_cpu_for_deletion = features_cpu
        del features_cpu_for_deletion
    ...
    def qda_objective(trial: optuna.Trial) -> float:
        ...
        # We use Dirichlet distribution to ensure the constraint
...
def main() -> None:
    ...
    # Cross-domain datasets as specified in the requirements
    datasets = [
        "resisc45",
        "imagenet-r", 
        "caltech-101",
