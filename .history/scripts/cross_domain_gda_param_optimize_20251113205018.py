...
from trainer import build_log_dirs, train_single_run, set_random
from utils.inc_net import BaseNet
from utils.data_manager import CrossDomainDataManagerCore
from torch.utils.data import DataLoader
from classifier.gaussian_classifier import compute_gaussian_statistics
...
def optimise_concatenated_datasets(
    ...
    all_features, all_labels = extract_concatenated_features(data_manager, args, model, eval_device)
    ...
    # Define the objective function for LDA
    def lda_objective(trial: optuna.Trial) -> float:
        ...
        classifier = builder.build(train_features, train_labels)
        accuracy = _evaluate_classifier(classifier, test_features, test_labels)
        metrics = {"lda": accuracy}
        trial.set_user_attr("metrics", metrics)
        del classifier
        return accuracy
    
    # Define the objective function for QDA
...
def extract_concatenated_features(data_manager, args, model, device):
    """
    Extract features from all cross-domain datasets and concatenate them.
    
    Args:
        data_manager: CrossDomainDataManagerCore instance
        args: Arguments dictionary
        model: Pre-trained model
        device: Device for feature extraction
    
    Returns:
        Tuple of (concatenated_features, concatenated_labels)
    """
    all_features = []
    all_labels = []
    ...
    for task_id in range(data_manager.nb_tasks):
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                task_features.append(features.cpu())
                task_labels.append(targets)
        
        # Concatenate all batches for this task
        task_features = torch.cat(task_features, dim=0)
        task_labels = torch.cat(task_labels, dim=0)
        
        # Adjust labels to be global (non-overlapping)
        offset = data_manager.global_label_offset[task_id]
        global_labels = task_labels + offset
        
        all_features.append(task_features)
        all_labels.append(global_labels)
        
        logging.info(f"Extracted {len(task_features)} features from task {task_id} ({data_manager.dataset_names[task_id]})")
    
    # Concatenate all features and labels
    concatenated_features = torch.cat(all_features, dim=0)
    concatenated_labels = torch.cat(all_labels, dim=0)
    
    logging.info(f"Total concatenated features: {len(concatenated_features)}")
    logging.info(f"Total concatenated labels: {len(concatenated_labels)}")
