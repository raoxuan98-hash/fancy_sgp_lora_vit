    ...
    args.update(
        {
            ...
            "num_shots": 64,  # 64-shot sampling as requested
...
def optimise_concatenated_datasets(
    ...
    def lda_objective(trial: optuna.Trial) -> float:
        # For LDA, we fix the identity matrix interpolation weight to 0.1
        # and only optimize the global covariance weight
        reg_alpha = trial.suggest_float("reg_alpha", lda_range[0], lda_range[1])
        
        # Build classifier using the builder pattern with fixed identity weight
        # The identity matrix interpolation weight is fixed at 0.1
        identity_weight = 0.1
        builder = LDAClassifierBuilder(reg_alpha=identity_weight, device=device_for_classifiers)
