    ...
    args.update(
        {
            ...
            "cross_domain": True,
            "cross_domain_datasets": ['imagenet-r', 'caltech-101'],
            ...
            "num_shots": 0,  # 64-shot sampling as requested
...
def optimise_concatenated_datasets(
    ...
    split_idx = int(0.5 * num_samples)  # 80% for training, 20% for testing
    ...
    # Create stats_dict for classifier builder using GaussianStatistics objects (compute once)
    logging.info("Computing Gaussian statistics for all classes...")
    stats_dict = {}
    ...
    for label in unique_labels:
        ...
        if len(class_features) > 0:
            ...
            cov = torch.cov(class_features.T) + 1e-4 * torch.eye(class_features.size(1), device=class_features.device)
            stats_dict[int(label)] = GaussianStatistics(mean, cov)
    
    # Move features to device if needed (similar to gda_param_optimize.py)
    if cache_features_on_device and eval_device.type != "cpu":
        features_for_objective = test_features.to(eval_device)
    else:
        features_for_objective = test_features
    ...
    def qda_objective(trial: optuna.Trial) -> float:
        ...
        accuracy = _evaluate_classifier(classifier, features_for_objective, test_labels)
