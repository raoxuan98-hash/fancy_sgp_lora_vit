...
# 1. 运行基础LoRA跨域实验 (GPU 0, num_incremental_splits=4)
{
    run_experiment_type "basic_lora" "basic_lora" "0" \
        "--gamma_kd" "0.0" \
        "--hopfield_temp" "0.1" \
        "--hopfield_topk" "200" \
        "--feature_combination_type" "aux_only" \
        "--auxiliary_data_size" "4096" \
        "--num_incremental_splits" "4"
} &
PIDS+=($!)
...
# 4. 运行LoRA-NSP跨域实验 (nsp_weight=0.03, nsp_eps=0.05) (GPU 4)
{
...
# 5. 运行LoRA-NSP跨域实验 (nsp_weight=0.03, nsp_eps=0.10) (GPU 5)
{
...
echo "=========================================="
echo "PHASE 2: Running Additional SGP LoRA Experiments"
echo "=========================================="

# 第二阶段：运行SGP LoRA实验，使用所有GPU
PIDS=()
...
# 8. 运行LoRA-SGP跨域实验 (weight_temp=1.5, weight_p=1.5) (GPU 2)
{
    run_experiment_type "sgp_lora_t1.5_p1.5" "sgp_lora" "2" \
        "--gamma_kd" "0.0" \
        "--weight_temp" "1.5" \
        "--weight_p" "1.5" \
        "--weight_kind" "log1p" \
...
# 9. 运行LoRA-SGP跨域实验 (weight_temp=0.8, weight_p=0.8) (GPU 4)
{
    run_experiment_type "sgp_lora_t0.8_p0.8" "sgp_lora" "4" \
        "--gamma_kd" "0.0" \
        "--weight_temp" "0.8" \
        "--weight_p" "0.8" \
        "--weight_kind" "log1p" \
...
# 10. 运行LoRA-SGP跨域实验 (weight_temp=2.5, weight_p=2.5) (GPU 5)
{
    run_experiment_type "sgp_lora_t2.5_p2.5" "sgp_lora" "5" \
        "--gamma_kd" "0.0" \
        "--weight_temp" "2.5" \
        "--weight_p" "2.5" \
        "--weight_kind" "log1p" \
...
# 计算总实验数量
TOTAL_EXPERIMENTS=$((${#SEEDS[@]} * 10))  # 10种实验变体
echo "Total experiments run: $TOTAL_EXPERIMENTS"
echo "Experiments per type: ${#SEEDS[@]} seeds (run sequentially)"
echo "Experiment variants: 10 (basic_lora, lora_kd_1.0, lora_kd_0.5, nsp_lora_0.05, nsp_lora_0.10, sgp_lora_t1.0_p1.0, sgp_lora_t2.0_p2.0, sgp_lora_t1.5_p1.5, sgp_lora_t0.8_p0.8, sgp_lora_t2.5_p2.5)"
echo "GPUs used: ${GPUS[@]} (5 GPUs available)"
echo "Execution strategy: Phased parallel execution with full GPU utilization"
echo "  - Phase 1: basic_lora, lora_kd_1.0, lora_kd_0.5, nsp_lora_0.05, nsp_lora_0.10 (5 GPUs)"
echo "  - Phase 2: sgp_lora_t1.0_p1.0, sgp_lora_t2.0_p2.0, sgp_lora_t1.5_p1.5, sgp_lora_t0.8_p0.8, sgp_lora_t2.5_p2.5 (5 GPUs)"
echo "Configuration: vit-b-p16-clip, enable_incremental_split=True, num_incremental_splits=2, evaluate_final_only=True"
echo "GPU allocation strategy: Optimized parallelization with balanced GPU utilization across all 5 GPUs"