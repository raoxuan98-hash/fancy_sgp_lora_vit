...
# 4. Full_NSP方法 (GPU 4)
{
    run_experiment "Full_NSP" "full_nsp" "4" "0.5" "${VIT_TYPES[0]}" "--lrate 5e-6 --update_teacher_each_task --nsp_eps 0.05 --kd_type feat --nsp_weight 0.0"
} &
PID_FULL_NSP=$!

# 5. Full + KD方法 (GPU 5)
{
    run_experiment "Full_KD" "full" "5" "1.0" "${VIT_TYPES[0]}" "--lrate 5e-6 --update_teacher_each_task --kd_type feat"
} &
PID_FULL_KD=$!
