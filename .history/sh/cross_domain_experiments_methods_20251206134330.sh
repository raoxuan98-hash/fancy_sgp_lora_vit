...
for VIT_TYPE in "${VIT_TYPES[@]}"; do
    ...
    # 6. Joint_Full方法 (GPU 0, 在Full完成后使用)
    {
        # 等待Full实验完成后再运行Joint_Full
        wait $PID_FULL
        run_experiment "Joint_Full_${VIT_TYPE}" "joint_full" "0" "0.0" "$VIT_TYPE" "--lrate 1e-5"
    } &
    PID_JOINT_FULL=$!
    ...
    wait $PID_JOINT_FULL
    echo "Joint_Full experiments completed for $VIT_TYPE"
...
echo "################################################################"
echo "All cross-domain experiments completed for all methods and architectures"
echo "################################################################"