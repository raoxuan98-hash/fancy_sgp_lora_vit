...
{
    run_experiment_type "basic_lora" "basic_lora" "0" "--gamma_kd" "0.0"
} &
PIDS+=($!)
...
# 2b. 运行LoRA + 蒸馏跨域实验 (gamma_kd=0.5) (GPU 2)
{
...
# 3. 运行LoRA-NSP跨域实验 (nsp_weight=0.05) (GPU 4)
{
...
# 3b. 运行LoRA-NSP跨域实验 (nsp_weight=0.00) (GPU 5)
{
...
# 等待前5个实验完成
echo "Waiting for first batch of experiments to complete..."
for PID in "${PIDS[@]}"; do
    wait $PID
done
...
# 4. 运行LoRA-SGP跨域实验 (weight_temp=1.0, weight_p=1.0) (GPU 0)
{
...
# 4b. 运行LoRA-SGP跨域实验 (weight_temp=2.0, weight_p=2.0) (GPU 1)
{
...
# 等待第二批实验完成
echo "Waiting for second batch of experiments to complete..."
for PID in "${PIDS[@]}"; do
    wait $PID
done
