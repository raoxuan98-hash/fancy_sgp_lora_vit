#!/usr/bin/env bash
set -euo pipefail

# 数据集列表
DATASETS=("cifar100_224" "imagenet-r" "cub200_224" "cars196_224")
SEEDS=(1993 1996 1997)

# 可用的GPU设备
GPUS=(0 1 2 4)

# 完整方法参数
LORA_TYPE="sgp_lora"
VIT_TYPE="vit-b-p16-mocov3"
GAMMA_KD=0.0  # 不使用蒸馏
WEIGHT_KIND="log1p"

# 参数网格测试
WEIGHT_TEMP_VALUES=(1.0 2.0 4.0)
WEIGHT_P_VALUES=(1.0 2.0)

# 创建日志目录
LOG_DIR="logs/full_method_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"

# 并行运行所有实验
PIDS=()
EXPERIMENT_COUNTER=0

for WEIGHT_TEMP in "${WEIGHT_TEMP_VALUES[@]}"; do
    for WEIGHT_P in "${WEIGHT_P_VALUES[@]}"; do
        for i in "${!DATASETS[@]}"; do
            DATASET="${DATASETS[$i]}"
            GPU="${GPUS[$((EXPERIMENT_COUNTER % ${#GPUS[@]}))]}"
            
            # 为每个数据集-参数组合创建一个子脚本来处理所有种子
            cat > "$LOG_DIR/run_${DATASET}_temp${WEIGHT_TEMP}_p${WEIGHT_P}.sh" << EOF
#!/usr/bin/env bash
set -euo pipefail

DATASET="$DATASET"
GPU="$GPU"
LOG_DIR="$LOG_DIR"
LORA_TYPE="$LORA_TYPE"
VIT_TYPE="$VIT_TYPE"
GAMMA_KD="$GAMMA_KD"
WEIGHT_TEMP="$WEIGHT_TEMP"
WEIGHT_P="$WEIGHT_P"
WEIGHT_KIND="$WEIGHT_KIND"
SEEDS=(${SEEDS[*]})

for SEED in "\${SEEDS[@]}"; do
    echo "Running Full Method experiment: dataset=\$DATASET, seed=\$SEED, weight_temp=\$WEIGHT_TEMP, weight_p=\$WEIGHT_P, GPU=\$GPU"
    
    CUDA_VISIBLE_DEVICES=\$GPU python -u main.py \\
        --dataset "\$DATASET" \\
        --smart_defaults \\
        --lora_type "\$LORA_TYPE" \\
        --vit_type "\$VIT_TYPE" \\
        --gamma_kd "\$GAMMA_KD" \\
        --weight_temp "\$WEIGHT_TEMP" \\
        --weight_p "\$WEIGHT_P" \\
        --weight_kind "\$WEIGHT_KIND" \\
        --seed_list "\$SEED" \\
        2>&1 | tee "\$LOG_DIR/\${DATASET}_temp\${WEIGHT_TEMP}_p\${WEIGHT_P}_seed\${SEED}.log"
done
EOF
            
            chmod +x "$LOG_DIR/run_${DATASET}_temp${WEIGHT_TEMP}_p${WEIGHT_P}.sh"
            
            # 在后台运行每个数据集-参数组合的实验
            echo "Starting experiments for $DATASET (temp=$WEIGHT_TEMP, p=$WEIGHT_P) on GPU $GPU"
            "$LOG_DIR/run_${DATASET}_temp${WEIGHT_TEMP}_p${WEIGHT_P}.sh" &
            PIDS+=($!)
            
            ((EXPERIMENT_COUNTER++))
        done
    done
done

# 等待所有实验完成
echo "Waiting for all experiments to complete..."
for PID in "${PIDS[@]}"; do
    wait $PID
done

echo "Full Method experiments completed. Logs saved to $LOG_DIR"
