...
echo "Starting all four methods experiments with parallel GPU execution..."

# 创建总日志目录
MASTER_LOG_DIR="logs/all_four_methods_$(date +%Y%m%d_%H%M%S)"
...
echo "=========================================="
echo "All four methods experiments completed!"
echo "Logs saved to: $MASTER_LOG_DIR"
echo "=========================================="

# 计算总实验数量
TOTAL_EXPERIMENTS=$((${#DATASETS[@]} * ${#SEEDS[@]} * 7))  # 7种实验变体
echo "Total experiments run: $TOTAL_EXPERIMENTS"
echo "Experiments per type: ${#DATASETS[@]} datasets × ${#SEEDS[@]} seeds = $((${#DATASETS[@]} * ${#SEEDS[@]}))"
echo "Experiment variants: 7 (basic_lora, lora_kd_1.0, lora_kd_0.5, nsp_lora_0.05, nsp_lora_0.00, sgp_lora_t1.0_p1.0, sgp_lora_t2.0_p1.0, sgp_lora_t2.0_p2.0)"