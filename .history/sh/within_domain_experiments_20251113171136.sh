...
echo "Starting Within-Domain Experiments..."

# 创建总日志目录
MASTER_LOG_DIR="logs/within_domain_experiments_$(date +%Y%m%d_%H%M%S)"
...
# 3b. 运行LoRA-NSP实验 (nsp_weight=0.00)
run_experiment_type "nsp_lora_0.00" "nsp_lora" \
    "--gamma_kd" "0.0" \
    "--nsp_weight" "0.00" \
    "--nsp_eps" "0.05"

# 4. 运行LoRA-SGP实验 (weight_temp=1.0, weight_p=1.0)
run_experiment_type "sgp_lora_t1.0_p1.0" "sgp_lora" \
    "--gamma_kd" "0.0" \
    "--weight_temp" "1.0" \
    "--weight_p" "1.0" \
    "--weight_kind" "log1p"

# 4b. 运行LoRA-SGP实验 (weight_temp=2.0, weight_p=1.0)
...
# 4c. 运行LoRA-SGP实验 (weight_temp=2.0, weight_p=2.0)
...
echo "=========================================="
echo "All Within-Domain experiments completed!"
echo "Logs saved to: $MASTER_LOG_DIR"
echo "=========================================="

# 计算总实验数量
TOTAL_EXPERIMENTS=$((${#DATASETS[@]} * ${#SEEDS[@]} * 8))  # 8种实验变体
echo "Total experiments run: $TOTAL_EXPERIMENTS"
echo "Experiments per type: ${#DATASETS[@]} datasets × ${#SEEDS[@]} seeds = $((${#DATASETS[@]} * ${#SEEDS[@]}))"
echo "Experiment variants: 8 (basic_lora, lora_kd_1.0, lora_kd_0.5, nsp_lora_0.05, nsp_lora_0.00, sgp_lora_t1.0_p1.0, sgp_lora_t2.0_p1.0, sgp_lora_t2.0_p2.0)"