...
class TestBalancedCrossDomainDataManagerWithRealData:
    ...
    def test_subset_creation(self):
        ...
        try:
            ...
            train_size = len(train_subset)  # Dataset应该实现__len__
            assert train_size > 0, f"训练子集应不为空，实际大小：{train_size}"
            
            # 测试测试子集
            test_subset = manager.get_subset(0, "test")
            test_size = len(test_subset)  # Dataset应该实现__len__
            assert test_size > 0, f"测试子集应不为空，实际大小：{test_size}"
            
            # 测试累积模式
            cumulative_subset = manager.get_subset(0, "train", cumulative=True)
            cum_size = len(cumulative_subset)  # Dataset应该实现__len__
            assert cum_size > 0, f"累积子集应不为空，实际大小：{cum_size}"
            
            # 测试数据加载
            sample = train_subset[0]
            assert len(sample) == 3, f"样本应有3个元素（图像、标签、类名），实际{len(sample)}"
            
            print(f"✅ 子集创建测试通过 (训练样本: {train_size}, 测试样本: {test_size})")
            return True
