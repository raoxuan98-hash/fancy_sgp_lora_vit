...
class TestBalancedCrossDomainDataManagerWithRealData:
    ...
    def test_incremental_splits(self):
        ...
        try:
            ...
            try:
                sample = subset[0]
                assert len(sample) == 3, f"增量子集样本应有3个元素，实际{len(sample)}"
                assert hasattr(subset, '__len__'), "Dataset应该有__len__方法"
            except Exception as e:
                raise AssertionError(f"增量子集访问失败: {str(e)}")
    ...
    def test_real_data_integration(self):
        ...
        try:
            ...
            for task_id in range(manager.nb_tasks):
                ...
                try:
                    sample = train_subset[0]
                    assert len(sample) == 3, f"任务{task_id}训练样本格式错误"
                    sample = test_subset[0]
                    assert len(sample) == 3, f"任务{task_id}测试样本格式错误"
                except Exception as e:
                    raise AssertionError(f"任务{task_id}子集访问失败: {str(e)}")
