...
def test_default_compensators():
    ...
    expected_variants = ["SeqFT", "SeqFT + linear", "SeqFT + weaknonlinear", "SeqFT + Hopfield"]
...
def test_partial_compensators():
    """测试只使用部分补偿器"""
    print("\n测试2: 只使用部分补偿器")
    partial_compensators = ["SeqFT", "SeqFT + linear"]
...
def test_argument_parsing_simple():
    ...
    comp.add_argument('--compensator_types', type=str, nargs='+',
                     default=['SeqFT', 'SeqFT + linear', 'SeqFT + weaknonlinear', 'SeqFT + Hopfield'],
    ...
    expected_default = ['SeqFT', 'SeqFT + linear', 'SeqFT + weaknonlinear', 'SeqFT + Hopfield']
    assert args.compensator_types == expected_default, f"默认补偿器参数不匹配: {args.compensator_types}"
    print("✓ 默认补偿器参数 - 通过")
    
    # 测试指定部分补偿器
    args = parser.parse_args(['--compensator_types', 'SeqFT', 'SeqFT + linear'])
    expected_partial = ['SeqFT', 'SeqFT + linear']
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n2. 只使用SeqFT和linear补偿器:")
        print("   python main.py --compensator_types SeqFT 'SeqFT + linear'")
        print("\n3. 只使用SeqFT补偿器:")
        print("   python main.py --compensator_types SeqFT")
        print("\n4. 使用SeqFT和Hopfield补偿器:")
        print("   python main.py --compensator_types SeqFT 'SeqFT + Hopfield'")
