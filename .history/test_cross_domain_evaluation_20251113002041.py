...
def setup_mock_model():
    ...
    mock_args = {
        ...
        'increment': 0,
        'seed': 42,
        # get_aux_loader 需要的参数
        'auxiliary_data_path': './data',
        'aux_dataset': 'cifar10'  # 使用 CIFAR-10 而不是 flickr8k
    }
    ...
    try:
        ...
        for pattern in expected_patterns:
            if pattern not in log_output:
                print(f"✗ 日志中缺少预期信息: {pattern}")
                return False
            else:
                print(f"✓ 日志包含预期信息: {pattern}")
        
        # 检查是否包含数据集名称
        for dataset_name in mock_data_manager.dataset_names:
            if dataset_name in log_output:
                print(f"✓ 日志包含数据集名称: {dataset_name}")
            else:
                print(f"✗ 日志缺少数据集名称: {dataset_name}")
                return False
...
def main():
    """主测试函数"""
    print("=" * 60)
    print("测试 models/subspace_lora.py 中的跨域评估功能")
    print("=" * 60)
    
    # 测试1: 验证代码是否能够正常运行，没有语法错误
    syntax_ok = test_syntax_errors()
    
    # 测试2: 检查 _evaluate_cross_domain() 方法是否正确输出每个数据集的平均准确度
    eval_ok = test_evaluate_cross_domain()
    
    # 测试3: 确认评估逻辑是否按数据集平均而不是按样本平均
    logic_ok = test_evaluation_logic()
    
    # 测试4: 验证输出格式是否清晰易读
    format_ok = test_output_format()
    
    # 总结测试结果
    print("\n" + "=" * 60)
    print("测试结果总结:")
    print(f"  语法检查: {'✓ 通过' if syntax_ok else '✗ 失败'}")
    print(f"  评估方法: {'✓ 通过' if eval_ok else '✗ 失败'}")
    print(f"  评估逻辑: {'✓ 通过' if logic_ok else '✗ 失败'}")
    print(f"  输出格式: {'✓ 通过' if format_ok else '✗ 失败'}")
    
    all_passed = syntax_ok and eval_ok and logic_ok and format_ok
