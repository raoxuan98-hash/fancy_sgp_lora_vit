...
def test_incremental_split_with_cross_domain():
    ...
    for i in range(manager.nb_tasks):
        ...
        if len(train_set) > 0:
            train_targets = []
            for i in range(min(len(train_set), 1000)):  # 最多检查1000个样本
                _, label, _ = train_set[i]
                train_targets.append(label)
            
            if train_targets:
                train_min = min(train_targets)
                train_max = max(train_targets)
                logger.info(f"  训练集: 样本数={len(train_set)}, 标签范围=[{train_min}, {train_max}] (基于样本检查)")
            
        if len(test_set) > 0:
            test_targets = []
            for i in range(min(len(test_set), 1000)):  # 最多检查1000个样本
                _, label, _ = test_set[i]
                test_targets.append(label)
            
            if test_targets:
                test_min = min(test_targets)
                test_max = max(test_targets)
                logger.info(f"  测试集: 样本数={len(test_set)}, 标签范围=[{test_min}, {test_max}] (基于样本检查)")
