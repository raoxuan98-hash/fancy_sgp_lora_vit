...
def test_training_simulation():
    ...
    for task_id in range(min(3, manager.nb_tasks)):  # 只测试前3个任务
        ...
        try:
            ...
            print(f"  ✅ 任务 {task_id} 模拟训练成功!")
            
        except Exception as e:
            print(f"  ❌ 任务 {task_id} 模拟训练失败: {str(e)}")
            return False
...
def main():
    ...
    print("开始测试增量拆分标签映射修复...")
    
    # 运行所有测试
    test1_passed = test_incremental_split_label_mapping()
    test2_passed = test_cumulative_mode()
    ...
    print("测试结果总结")
    print("=" * 80)
    print(f"标签映射测试: {'✅ 通过' if test1_passed else '❌ 失败'}")
    print(f"累积模式测试: {'✅ 通过' if test2_passed else '❌ 失败'}")
    print(f"训练模拟测试: {'✅ 通过' if test3_passed else '❌ 失败'}")
