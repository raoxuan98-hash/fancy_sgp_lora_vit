...
def test_sgp_lora_naming():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "t-2p0" in log_path or "t-2.0" in log_path, f"weight_temp not found in {log_path}"
        assert "k-log1p" in log_path, f"weight_kind not found in {log_path}"
        assert "p-1" in log_path or "p-1.0" in log_path, f"weight_p not found in {log_path}"
...
def test_nsp_lora_naming():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "eps-0p05" in log_path or "eps-0.05" in log_path, f"nsp_eps not found in {log_path}"
        assert "w-0p02" in log_path or "w-0.02" in log_path, f"nsp_weight not found in {log_path}"
