...
def train(args):
    ...
    for run_id, seed in enumerate(args['seed_list']):
        ...
        # 保存共享目录路径，用于聚合结果
        if shared_log_dir is None:
            shared_log_dir = Path(logfile_name).parent
    ...
    if shared_log_dir is not None:
        ...
        # 将共享目录路径添加到结果中，以便聚合函数使用
        for seed_key, seed_result in all_results.items():
            if isinstance(seed_result, dict):
                seed_result['shared_log_dir'] = str(shared_log_dir)
...
def aggregate_seed_results(seed_results):
    ...
    # 新增：收集每个任务的准确度列表
    per_task_accuracies = {variant: [] for variant in all_variants}
    max_tasks = 0  # 记录最大任务数
    ...
    for res in records:
        ...
        per_task = res.get("per_task_results", {})
        
        # 更新最大任务数
        if per_task:
            max_tasks = max(max_tasks, max(per_task.keys()) + 1)  # +1因为任务ID从0开始
        ...
        for variant in all_variants:
            ...
            if per_task:
                ...
                for task_id in sorted(per_task.keys()):
                    task_accs.append(per_task[task_id].get(variant, 0.0))
                per_task_accuracies[variant].append(task_accs)
    ...
    for variant in all_variants:
        ...
        # 计算每个任务的平均准确度和标准差
        if per_task_accuracies[variant]:
            # 将所有种子的任务准确度转换为numpy数组
            task_arrays = np.array(per_task_accuracies[variant])
            # 计算每个任务的平均值和标准差
    ...
    # 新增：记录每个任务的准确度趋势
    logging.info("  ── Per-Task Accuracy Trends (Mean ± Std) ──")
    for variant in all_variants:
        if variant in per_task_stats:
            ...
            logging.info(f"      {variant:<20} : {trend_str}")

    # === 🗂️ SAVE AGGREGATED RESULTS TO FILE ===
    # 保存聚合结果到JSON文件
    import time
    
    # 尝试从第一个种子的结果中获取共享目录路径
    shared_dir = None
    if isinstance(seed_results, Mapping) and len(seed_results) > 0:
        ...
        # 首先尝试获取shared_log_dir
        if isinstance(first_seed_result, dict) and 'shared_log_dir' in first_seed_result:
            shared_dir = Path(first_seed_result['shared_log_dir'])
        else:
            # 如果没有shared_log_dir，尝试从log_path获取
            log_path = None
            ...
            if log_path:
                # 获取种子目录的父目录，即所有种子共享的目录
                seed_dir = Path(log_path)
                shared_dir = seed_dir.parent
        
        if shared_dir:
            aggregate_file = shared_dir / "aggregate_results.json"
            ...
            save_data = {
                ...
                "per_task_accuracy_trends": per_task_stats,  # 新增：每个任务的准确度趋势
                "seed_list": list(seed_results.keys()),
                "num_seeds": len(seed_results),
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "variants": all_variants,
                "max_tasks": max_tasks  # 新增：最大任务数
            }
            ...
            logging.warning("⚠️ Could not find shared directory for saving aggregated results.")
    else:
    ...
    return {
        "final_task": final_task_stats,
        "average_across_tasks": avg_task_stats,
        "per_task_accuracy_trends": per_task_stats,  # 新增：返回每个任务的准确度趋势
    }
