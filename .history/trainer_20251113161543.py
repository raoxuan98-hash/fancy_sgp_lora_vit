...
def build_log_dirs(args: dict, root_dir="."):
    ...
    if is_cross_domain:
        ...
        if 'cross_domain_datasets' in args:
            # 将跨域数据集列表转换为简短的字符串标识
            datasets_str = "_".join([short(ds) for ds in args['cross_domain_datasets']])
            task_dir = os.path.join(
                base_dir,
                "cross_domain",  # 明确标识跨域实验
                f"{datasets_str}_{short(args['vit_type'])}",
            ...
            task_dir = os.path.join(
                base_dir,
                "cross_domain",
                f"{short(args.get('dataset', 'unknown'))}_{short(args['vit_type'])}",
                f"shots-{short(args.get('num_shots', 0))}",
                f"lrank-{short(args.get('lora_rank', 'NA'))}_ltype-{short(args.get('lora_type', 'NA'))}"
            )
    else:
...
def analyze_all_results(all_results: dict, dataset_names: list = None, save_json: bool = True, output_path: str = None) -> dict:
            ...
            for task_id in task_ids:
                ...
                for seed_key in seed_keys:
                    ...
                    if ('per_task_results' in seed_result and
                        ...
                        task_accs.append(seed_result['per_task_results'][task_id][variant])
    ...
    if save_json:
        if output_path is None:
            ...
            output_path = log_dir / "multi_seed_statistics.json"
