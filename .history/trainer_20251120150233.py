...
def analyze_all_results(all_results: dict, dataset_names: list = [], save_json: bool = True, output_path: str = "") -> dict:
    ...
    # 从per_task_results获取变体名称（只获取数值类型的键）
    ...
    for variant in variant_names:
        ...
        # 从per_task_results中提取_class_wise_data数据
        class_wise_accs = []
        for seed_key in seed_keys:
            seed_result = all_results[seed_key]
            # 检查per_task_results中是否有_class_wise_data
            if 'per_task_results' in seed_result:
                # 从最后一个任务的_class_wise_data中获取class_wise_accuracies
                last_task_id = max(seed_result['per_task_results'].keys()) if seed_result['per_task_results'] else None
                if last_task_id and last_task_id in seed_result['per_task_results']:
                    task_result = seed_result['per_task_results'][last_task_id]
                    if '_class_wise_data' in task_result and 'class_wise_accuracies' in task_result['_class_wise_data']:
                        class_wise_data = task_result['_class_wise_data']['class_wise_accuracies']
                        if variant in class_wise_data:
                            class_wise_accs.append(class_wise_data[variant])
        ...
        for seed_key in seed_keys:
            seed_result = all_results[seed_key]
            if 'per_task_results' in seed_result:
                # 收集所有任务的class-wise平均准确度
                seed_class_wise_accs = []
                for task_id, task_result in seed_result['per_task_results'].items():
                    if '_class_wise_data' in task_result and 'per_task_class_wise_accuracies' in task_result['_class_wise_data']:
                        per_task_data = task_result['_class_wise_data']['per_task_class_wise_accuracies']
                        if variant in per_task_data and per_task_data[variant]:
                            # 获取当前任务的class-wise平均准确度（最后一个值）
                            task_class_wise_acc = per_task_data[variant][-1]
                            seed_class_wise_accs.append(task_class_wise_acc)
                
                # 如果该种子有class-wise数据，计算平均值
                if seed_class_wise_accs:
                    average_class_wise_accs.append(np.mean(seed_class_wise_accs))
        ...
        if task_ids:
            ...
            first_seed_result = all_results[seed_keys[0]]
            per_task_accs_found = False
            
            for task_id in task_ids:
                if task_id in first_seed_result['per_task_results']:
                    task_result = first_seed_result['per_task_results'][task_id]
                    if '_class_wise_data' in task_result and 'per_task_class_wise_accuracies' in task_result['_class_wise_data']:
                        per_task_data = task_result['_class_wise_data']['per_task_class_wise_accuracies']
                        if variant in per_task_data and per_task_data[variant]:
                            dataset_name = dataset_names[task_id - 1] if dataset_names and task_id - 1 < len(dataset_names) else f"Task {task_id}"
                            task_class_acc = per_task_data[variant][-1]  # 最后一个值是当前任务的class-wise平均准确度
                            logging.info(f"    {dataset_name}: {task_class_acc:.2f}%")
                            per_task_accs_found = True
            
            if not per_task_accs_found:
