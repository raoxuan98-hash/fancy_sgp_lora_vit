    ...
    for variant in variant_names:
        if variant_stats := statistics_results["variants"][variant]:
            if "last_task_accuracy" in variant_stats and "mean" in variant_stats["last_task_accuracy"]:
                ranked_by_last_task.append((variant, variant_stats["last_task_accuracy"]["mean"]))
    
    if ranked_by_last_task:
        ranked_by_last_task.sort(key=lambda x: x[1], reverse=True)
        logging.info("📈 按最后任务准确率排名:")
        for i, (variant, acc) in enumerate(ranked_by_last_task, 1):
    ...
    for variant in variant_names:
        if variant_stats := statistics_results["variants"][variant]:
            if "average_accuracy" in variant_stats and "mean" in variant_stats["average_accuracy"]:
                ranked_by_avg.append((variant, variant_stats["average_accuracy"]["mean"]))
    
    if ranked_by_avg:
        ranked_by_avg.sort(key=lambda x: x[1], reverse=True)
        logging.info("\n📊 按平均准确率排名:")
        for i, (variant, acc) in enumerate(ranked_by_avg, 1):
            logging.info(f"  {i:2d}. {variant:<30}: {acc:.2f}%")
