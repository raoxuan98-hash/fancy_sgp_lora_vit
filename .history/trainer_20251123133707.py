...
def build_log_dirs(args: dict, root_dir="."):
    ...
    def _get_auxiliary_params(args: dict) -> list:
        ...
        # 添加feature_combination_type参数到日志命名
        if 'feature_combination_type' in args:
            aux_params.append(f"fc-{short(args['feature_combination_type'])}")
        
        # 添加auxiliary_data_size参数到日志命名
        if 'auxiliary_data_size' in args:
            aux_params.append(f"aux-{short(args['auxiliary_data_size'])}")
    ...
    def _get_incremental_split_params(args: dict) -> list:
        ...
        # 添加enable_incremental_split参数
        if args.get('enable_incremental_split', False):
            inc_params.append("inc_split-enabled")
            # 如果启用了增量拆分，也添加num_incremental_splits参数
            num_splits = args.get('num_incremental_splits', 2)
            inc_params.append(f"splits-{short(num_splits)}")
        else:
            inc_params.append("inc_split-disabled")
    ...
    # 合并 LoRA、KD、辅助数据和增量拆分参数
    method_params = lora_params + kd_params + aux_params + inc_split_params
