...
def print_args(args):
    ...
    # 特殊处理权重插值参数，确保其清晰显示
    if 'enable_weight_interpolation' in args:
        if args['enable_weight_interpolation']:
            alpha = args.get('interpolation_alpha', 1.0)
            if alpha < 1.0:
                logging.info(f'⚙️ 权重插值已启用: alpha={alpha}')
                if alpha == 0.0:
                    logging.info(f'   └─ 完全使用prev_network参数')
                elif alpha == 0.5:
                    logging.info(f'   └─ 对等权重插值')
                elif alpha == 1.0:
                    logging.info(f'   └─ 完全使用当前网络参数')
                else:
                    logging.info(f'   └─ 自定义插值: {alpha*100:.1f}% 当前网络 + {(1-alpha)*100:.1f}% prev_network')
            else:
                logging.info(f'⚙️ 权重插值已启用但alpha={alpha}，相当于禁用插值')
        else:
            logging.info(f'⚙️ 权重插值未启用')
