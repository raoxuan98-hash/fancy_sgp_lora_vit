...
class BalancedCrossDomainDataManagerCore(CrossDomainDataManagerCore):
    ...
    def __init__(
        ...
        if use_balanced_datasets:
            # 使用平衡后的数据集
            self._init_balanced_datasets(dataset_names, shuffle, seed, num_shots, 
                                     num_samples_per_task_for_evaluation, log_level)
        else:
            # 使用原始数据集
            super().__init__(dataset_names, shuffle, seed, num_shots, 
                           num_samples_per_task_for_evaluation, log_level)
    
    def _init_balanced_datasets(self, dataset_names: List[str], shuffle: bool, seed: int, 
                              num_shots: int, num_samples_per_task_for_evaluation: int, 
                              log_level: int):
        """初始化平衡后的数据集"""
        ...
        for i, dataset_name in enumerate(dataset_names):
            ...
            if dataset_info is None:
                ...
                dataset_info = {
                    'name': dataset_name,
                    'train_data': np.asarray(original_dataset.train_data),
                    'test_data': np.asarray(original_dataset.test_data),
                    'train_targets': np.asarray(original_dataset.train_targets, dtype=np.int64),
                    'test_targets': np.asarray(original_dataset.test_targets, dtype=np.int64),
                    'num_classes': len(np.unique(original_dataset.train_targets)),
    ...
    def compare_with_original(self) -> Dict[str, Dict]:
        ...
        for dataset_name in self.dataset_names:
            try:
                ...
                for label in original_dataset.train_targets:
                    original_train_counts[label] = original_train_counts.get(label, 0) + 1
                
                for label in original_dataset.test_targets:
                    original_test_counts[label] = original_test_counts.get(label, 0) + 1
                ...
                for label in balanced_dataset['train_targets']:
                    balanced_train_counts[label] = balanced_train_counts.get(label, 0) + 1
                
                for label in balanced_dataset['test_targets']:
                    balanced_test_counts[label] = balanced_test_counts.get(label, 0) + 1
                
                comparison[dataset_name] = {
                    'original': {
                        'total_train_samples': len(original_dataset.train_data),
                        'total_test_samples': len(original_dataset.test_data),
                        'train_per_class_stats': {
                            'min': min(original_train_counts.values()),
                            'max': max(original_train_counts.values()),
                            'mean': np.mean(list(original_train_counts.values())),
                            'std': np.std(list(original_train_counts.values()))
                        },
                        'test_per_class_stats': {
                            'min': min(original_test_counts.values()),
                            'max': max(original_test_counts.values()),
                            'mean': np.mean(list(original_test_counts.values())),
                            'std': np.std(list(original_test_counts.values()))
                        }
                    },
                    'balanced': {
                        ...
                        'train_per_class_stats': {
                            'min': min(balanced_train_counts.values()),
                            'max': max(balanced_train_counts.values()),
                            'mean': np.mean(list(balanced_train_counts.values())),
                            'std': np.std(list(balanced_train_counts.values()))
                        },
                        'test_per_class_stats': {
                            'min': min(balanced_test_counts.values()),
                            'max': max(balanced_test_counts.values()),
                            'mean': np.mean(list(balanced_test_counts.values())),
                            'std': np.std(list(balanced_test_counts.values()))
                        }
                    }
                }
