...
class BalancedCrossDomainDataManagerCore(CrossDomainDataManagerCore):
    ...
    def _create_incremental_dataset(self, original_dataset: Dict, class_indices: List[int],
                                   split_idx: int) -> Dict:
        # 获取原始数据集的全局偏移
        # 注意：original_dataset 的标签已经被加上了全局偏移，所以我们需要先减去这个偏移
        global_offset = None
        for i, dataset in enumerate(self.datasets):
            if dataset['name'] == original_dataset['name']:
                # 找到对应的数据集索引
                if i < len(self.global_label_offset):
                    global_offset = self.global_label_offset[i]
                break

        
        # 将 class_indices 转换为全局标签（加上全局偏移）
        global_class_indices = [idx + global_offset for idx in class_indices]
        
        # 筛选训练数据
        train_mask = np.isin(original_dataset['train_targets'], global_class_indices)
        train_data = original_dataset['train_data'][train_mask]
        train_targets_original = original_dataset['train_targets'][train_mask]
        
        # 筛选测试数据
        test_mask = np.isin(original_dataset['test_targets'], global_class_indices)
        test_data = original_dataset['test_data'][test_mask]
        test_targets_original = original_dataset['test_targets'][test_mask]
        
        # 创建标签映射：全局标签 -> 新的局部标签（从0开始）
        label_mapping = {orig_label: new_label + global_offset for new_label, orig_label in enumerate(sorted(global_class_indices))}
        
        # 应用标签映射，将标签映射为从0开始的连续标签
        train_targets = np.array([label_mapping[t] for t in train_targets_original])
        test_targets = np.array([label_mapping[t] for t in test_targets_original])
        ...
        split_dataset = {
            ...
            'test_data': test_data,
            'train_targets': train_targets,  # 使用重新映射的局部标签（从0开始）
            'test_targets': test_targets,    # 使用重新映射的局部标签（从0开始）
