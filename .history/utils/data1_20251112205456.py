...
class SimpleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # DEBUG: 添加调试日志以验证标签和 class_names 的范围
        if self.class_names is not None:
            if label >= len(self.class_names):
                print(f"ERROR: Label {label} out of range for class_names of length {len(self.class_names)}")
                print(f"Max valid label: {len(self.class_names) - 1}")
                print(f"Dataset info - images: {len(self.images)}, labels: {len(self.labels)}")
                print(f"First 10 labels: {self.labels[:10].tolist() if len(self.labels) >= 10 else self.labels.tolist()}")
                print(f"Label range: min={min(self.labels)}, max={max(self.labels)}")
                raise IndexError(f"Label {label} out of range for class_names of length {len(self.class_names)}")
