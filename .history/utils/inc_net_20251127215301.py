...
class RandomNet(nn.Module):
    def __init__(self, args, pretrained):
        super().__init__()
        self.vit = get_vit(args, pretrained)
        if args['randmo_projection'] <= 0:
            self.random_projector = nn.Identity()
            self.random_projection_dim = self.vit.feature_dim

        elif args['randmo_projection']:
            self.random_projection_dim = args['random_projection_dim']
            self.random_projector = nn.Sequential(nn.Linear(self.vit.feature_dim, self.random_projection_dim), nn.ReLU())
