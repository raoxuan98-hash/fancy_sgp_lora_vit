...
class RandomNet(nn.Module):
    def __init__(self, args, pretrained):
        super().__init__()
        self.vit = get_vit(args, pretrained)
        self.orig_feature_dim = getattr(self.vit, 'feature_dim', 768)

        if args['random_projection_dim'] <= 0:
            # 不使用 RFF，直接输出 ViT 特征
            self.use_rff = False
            self.random_projection_dim = self.orig_feature_dim
        else:
            # 启用 RFF
            self.use_rff = True
            self.random_projection_dim = args['random_projection_dim']
            D = self.random_projection_dim
            d = self.orig_feature_dim

            # 注册固定参数（不参与梯度更新）
            self.register_buffer('omega', torch.randn(D, d))
            self.register_buffer('bias', torch.rand(D) * 2 * torch.pi)
    ...
    def _rff_map(self, x):
        ...
        x: (..., d) → (..., D)
        """
        # 可选：对输入做 L2 归一化（推荐，尤其 ViT 特征）
        x = F.normalize(x, dim=-1)
        # Compute Ωx + b
        proj = F.linear(x, self.omega, self.bias)  # (..., D)
        # Apply cos and scale
        phi = torch.cos(proj) * (2.0 / self.random_projection_dim) ** 0.5
        return phi

    def forward_features(self, x, random_projection=True):
        feat = self.vit(x)  # (B, d)
        if random_projection and self.use_rff:
            feat = self._rff_map(feat)  # (B, D)
            feat = torch.cat([feat, feat], dim=-1)
        return feat

    @property
    def feature_dim(self):
        return self.random_projection_dim + self.orig_feature_dim

    def update_fc(self, nb_classes):
        if self.fc is None:
            # 注意：分类头仍作用于原始 ViT 特征（非 RFF 输出）
            # 若想用 RFF 特征分类，需改为 self.random_projection_dim
            self.fc = ContinualLinear(self.orig_feature_dim, nb_classes)
        else:
            self.fc.update(nb_classes)