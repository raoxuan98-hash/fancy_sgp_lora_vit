...
class BoxCoxTransform(nn.Module):
    def __init__(self, lam, eps=1e-6):
        super().__init__()
        self.lam = lam
        self.eps = eps  # 避免除零或 log(0)

    def forward(self, x):
        # 确保输入为正
        x = x + self.eps  # x >= eps > 0
        
        if abs(self.lam) < 1e-6:  # lambda ≈ 0 → log(x)
            return torch.log(x)
        else:
            return (x ** self.lam - 1.0) / self.lam
