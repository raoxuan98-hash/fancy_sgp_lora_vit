...
# 获取完整的训练数据集
full_train_subsets = dataset.get_subset(len(cross_domain_datasets)-1, source='train', cumulative=True, mode="train")
...
# 计算分割比例 - 使用80%作为训练，20%作为测试
total_size = len(full_train_subsets)
train_size = int(0.8 * total_size)
test_size = total_size - train_size

# 分割数据集
train_subsets, test_subsets = random_split(full_train_subsets, [train_size, test_size])
...
# 使用测试数据评估分类器
acc = evaluate_qda_classifier(0.10, train_stats, test_features, test_labels, device="cuda")
print(f"修复后的准确率: {acc}")
