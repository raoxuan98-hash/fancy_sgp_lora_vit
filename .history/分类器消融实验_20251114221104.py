...
def extract_features_and_labels(model, test_loader):
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(test_loader):
            ...
            # 提取数据集信息
            if len(batch) > 2:
                # 如果数据集返回了class_name，我们可以用它来推断数据集
                class_names = batch[2]
                # 根据class_name推断数据集ID
                batch_dataset_ids = [infer_dataset_id(class_name) for class_name in class_names]
                dataset_ids.extend(batch_dataset_ids)
...
def infer_dataset_id(class_name):
    """根据类别名称推断数据集ID"""
    # 这里需要根据实际的类别名称到数据集的映射关系来实现
    # 由于我们不知道确切的映射关系，这里提供一个简单的实现
    ...
    # 简单的启发式规则，需要根据实际情况调整
    if 'car' in class_name_str:
        return 9  # cars196_224
    elif 'aircraft' in class_name_str:
        return 4  # fgvc-aircraft-2013b-variants102
    elif 'bird' in class_name_str or 'cub' in class_name_str:
        return 2  # cub200_224 (如果存在)
    elif 'dog' in class_name_str or 'pet' in class_name_str:
        return 8  # oxford-iiit-pets
    elif 'flower' in class_name_str:
        return 7  # oxford-flower-102
    elif 'food' in class_name_str:
        return 5  # food-101
    elif 'mnist' in class_name_str or 'digit' in class_name_str:
        return 6  # mnist
    elif 'texture' in class_name_str or 'dtd' in class_name_str:
        return 3  # dtd
    elif 'caltech' in class_name_str:
        return 2  # caltech-101
    elif 'imagenet' in class_name_str or 'r' in class_name_str:
        return 1  # imagenet-r
    else:
        return 0  # resisc45 (默认)
...
def evaluate_qda_classifier(alpha1, stats, features, targets, dataset_ids, device="cuda", batch_size=512):
    """
    评估QDA分类器的准确率，使用dataset-wise的平均准确率而不是所有样本的平均准确率
    
    参数:
        alpha1: QDA正则化参数alpha1
        stats: 训练数据的高斯统计量
        features: 测试特征
        targets: 测试标签
        dataset_ids: 每个样本所属的数据集ID
        device: 计算设备
        batch_size: 批处理大小
    
    返回:
        dataset_wise_accuracy: 按数据集计算的平均准确率
    """
    alpha2 = 0.95 - alpha1
    alpha3 = 0.05
...
def plot_alpha1_accuracy_curve(alpha1_values, accuracies, save_path=None):
    ...
    plt.xlabel('Alpha1 值')
    plt.ylabel('Dataset-wise 准确率')
    plt.title('Alpha1 到 Dataset-wise 准确率的函数曲线')
