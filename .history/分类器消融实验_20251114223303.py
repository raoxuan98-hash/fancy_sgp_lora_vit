...
# %%
# 控制是否在训练集上适应网络主干
adapt_backbone = True  # 设置为False则不适应，True则适应500个iterations
...
if adapt_backbone:
    ...
    adapt_dataloader = torch.utils.data.DataLoader(train_subsets, batch_size=16, shuffle=True, num_workers=8)
    optimizer = torch.optim.Adam(list(vit.parameters()) + list(classifier.parameters()), lr=1e-5)
