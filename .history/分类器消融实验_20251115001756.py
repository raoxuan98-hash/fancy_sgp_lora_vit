...
def evaluate_qda_classifier(alpha1, alpha3, stats, features, targets, dataset_ids, device="cuda", batch_size=512):
    alpha2 = 1 - alpha1  # 根据要求，alpha_2固定为1 - alpha_1
...
def grid_search_alpha1_alpha3(alpha1_min=0, alpha1_max=0.4, alpha3_min=0, alpha3_max=1.0,
                           alpha1_points=10, alpha3_points=10):
    """
    对alpha1和alpha3进行二维网格搜索
    
    参数:
        alpha1_min: alpha1的最小值
        alpha1_max: alpha1的最大值
        alpha3_min: alpha3的最小值
        alpha3_max: alpha3的最大值
        alpha1_points: alpha1的测试点数
        alpha3_points: alpha3的测试点数
    
    返回:
        alpha1_values: alpha1值网格
        alpha3_values: alpha3值网格
        accuracy_matrix: 准确率矩阵
    """
    # 生成alpha1和alpha3的值网格
    alpha1_values = np.linspace(alpha1_min, alpha1_max, alpha1_points)
    alpha3_values = np.linspace(alpha3_min, alpha3_max, alpha3_points)
    
    # 初始化准确率矩阵
    accuracy_matrix = np.zeros((alpha1_points, alpha3_points))
    
    print(f"开始二维网格搜索: {alpha1_points} x {alpha3_points} = {alpha1_points * alpha3_points} 个组合")
    
    total_tests = alpha1_points * alpha3_points
    test_count = 0
    
    for i, alpha1 in enumerate(alpha1_values):
        for j, alpha3 in enumerate(alpha3_values):
            test_count += 1
            print(f"\n测试进度: {test_count}/{total_tests}, alpha1={alpha1:.4f}, alpha3={alpha3:.4f}")
            
            # 评估当前参数组合
            acc = evaluate_qda_classifier(alpha1, alpha3, train_stats, test_features,
                                         test_labels, test_dataset_ids, device="cuda")
            accuracy_matrix[i, j] = acc
            print(f"Dataset-wise准确率: {acc:.4f}")
            print("-" * 50)
    
    return alpha1_values, alpha3_values, accuracy_matrix

def plot_alpha1_alpha3_contour(alpha1_values, alpha3_values, accuracy_matrix, save_path=None):
    """
    绘制alpha1和alpha3的二维等高线图
    
    参数:
        alpha1_values: alpha1值列表
        alpha3_values: alpha3值列表
        accuracy_matrix: 准确率矩阵
    ...
    # 创建网格
    alpha1_grid, alpha3_grid = np.meshgrid(alpha1_values, alpha3_values)
    
    # 绘制等高线图
    contour = plt.contourf(alpha1_grid, alpha3_grid, accuracy_matrix.T, levels=20, cmap='viridis')
    plt.colorbar(contour, label='Dataset-wise Avg-Acc')
    
    # 添加等高线
    contour_lines = plt.contour(alpha1_grid, alpha3_grid, accuracy_matrix.T, levels=10, colors='black', alpha=0.4)
    plt.clabel(contour_lines, inline=True, fontsize=8)
    
    # 找到最佳准确率及其对应的参数
    max_idx = np.unravel_index(np.argmax(accuracy_matrix), accuracy_matrix.shape)
    best_alpha1 = alpha1_values[max_idx[0]]
    best_alpha3 = alpha3_values[max_idx[1]]
    best_acc = accuracy_matrix[max_idx]
    
    # 标记最佳点
    plt.plot(best_alpha1, best_alpha3, 'r*', markersize=15, label=f'Best: ({best_alpha1:.3f}, {best_alpha3:.3f}) = {best_acc:.4f}')
    
    plt.xlabel('Alpha1')
    plt.ylabel('Alpha3')
    plt.title('Alpha1-Alpha3 Grid Search Results')
    plt.legend()
    ...
    plt.show()
    return best_alpha1, best_alpha3, best_acc
...
# %%
# 执行二维网格搜索并绘制等高线图
alpha1_values, alpha3_values, accuracy_matrix = grid_search_alpha1_alpha3(
    alpha1_min=0, alpha1_max=0.4, alpha3_min=0, alpha3_max=1.0,
    alpha1_points=10, alpha3_points=10
)

best_alpha1, best_alpha3, best_acc = plot_alpha1_alpha3_contour(
    alpha1_values, alpha3_values, accuracy_matrix,
    save_path="alpha1_alpha3_contour.png"
)

print(f"\n最终结果:")
print(f"最佳alpha1值: {best_alpha1:.4f}")
print(f"最佳alpha3值: {best_alpha3:.4f}")
print(f"对应的Dataset-wise准确率: {best_acc:.4f}")
print(f"对应的alpha2值: {1 - best_alpha1:.4f}")
# %%
