...
# 放在 .py 文件的顶层（不要缩进！）
def worker(args):
    i, j, alpha1, alpha2, alpha3_fixed, train_stats, test_features, test_labels, test_dataset_ids = args
    acc = evaluate_qda_classifier(
        alpha1, alpha2, alpha3_fixed, train_stats, test_features,
        test_labels, test_dataset_ids, device="cuda"
    )
    return i, j, acc

def grid_search_alpha1_alpha2_batch(alpha1_min=0, alpha1_max=1.0, alpha2_min=0, alpha2_max=1.0,
    ...
    # 在 grid_search 函数中替换循环部分
    args_list = []
    for i, alpha1 in enumerate(alpha1_values):
        for j, alpha2 in enumerate(alpha2_values):
            args_list.append((i, j, alpha1, alpha2, alpha3_fixed, train_stats, test_features, test_labels, test_dataset_ids))

    accuracy_matrix = np.zeros((alpha1_points, alpha2_points))
    with ProcessPoolExecutor(max_workers=4) as executor:  # 根据 GPU 内存调整 workers 数量
        futures = [executor.submit(worker, args) for args in args_list]
        for future in as_completed(futures):
            i, j, acc = future.result()
            accuracy_matrix[i, j] = acc
            print(f"Completed: alpha1={alpha1_values[i]:.4f}, alpha2={alpha2_values[j]:.4f}, acc={acc:.4f}")
...
# %%
# 执行二维网格搜索并绘制等高线图
alpha1_values, alpha2_values, accuracy_matrix = grid_search_alpha1_alpha2_batch(
