...
import numpy as np
import matplotlib
# IEEE单栏风格设置：使用Type1字体，避免Type3字体
...
dataset = CrossDomainDataManagerCore(cross_domain_datasets, shuffle=False, seed=0, num_shots=128)
...
def extract_features_and_labels(model, loader, cache_dir="cached_data/classifier_ablation"):
    """
    提取特征和标签，支持缓存功能
    """
    # 检查缓存
    model_name = "vit-b-p16-clip"
    cache_key = f"{model_name}_features_cache"
    cache_path = os.path.join(cache_dir, cache_key)
    
    if os.path.exists(cache_path + "_train_features.pt") and os.path.exists(cache_path + "_train_labels.pt"):
    ...
    train_dataset_ids = infer_dataset_ids_from_labels(train_labels, dataset)
    ...
    test_loader = torch.utils.data.DataLoader(test_subsets, batch_size=64, shuffle=False, num_workers=8)
    ...
    test_dataset_ids = infer_dataset_ids_from_labels(test_labels, dataset)
...
train_features, train_labels, train_dataset_ids, test_features, test_labels, test_dataset_ids = extract_features_and_labels(vit, loaders)
...
# 修改evaluate_qda_classifier函数，支持自定义分类器
# 删除旧的evaluate_qda_classifier函数定义（在第184行之前）
# 新的evaluate_qda_classifier函数已经在文件末尾定义
...
def plot_alpha1_alpha2_contour(alpha1_values, alpha2_values, accuracy_matrix, save_path=None):
    """
    绘制alpha1和alpha2的二维等高线图
    
    参数:
        alpha1_values: alpha1值列表
        alpha2_values: alpha2值列表
    ...
    # 标记最佳点
    plt.plot(best_alpha1, best_alpha2, 'r*', markersize=15, label=f'Best: ({best_alpha1:.3f}, {best_alpha2:.3f}) = {best_acc:.4f}')
    
    plt.xlabel('Alpha1')
    plt.ylabel('Alpha2')
    plt.title('Alpha1-Alpha2 Grid Search Results (Alpha3=0.2)')
    plt.legend()
...
best_alpha1, best_alpha2, best_acc = plot_alpha1_alpha2_contour(
    ...
    save_path="alpha1_alpha2_contour.png"
)
...
def plot_experiment2_sensitivity(results, save_path=None):
    ...
    ax.set_xlabel('Alpha1 (Alpha1+Alpha2=Constant)')
    ax.set_ylabel('Dataset-wise Avg-Acc')
    ax.set_title('Fixed Sum')
    ...
    ax.set_xlabel('Alpha2 (Alpha1=1.0)')
    ax.set_ylabel('Dataset-wise Avg-Acc')
    ax.set_title('Fixed Alpha1')
    ax.grid(True, linestyle='--', alpha=0.7)
    ...
    ax.set_xlabel('Alpha1 (Alpha2=2.0)')
    ax.set_ylabel('Dataset-wise Avg-Acc')
    ax.set_title('Fixed Alpha2')
    ax.grid(True, linestyle='--', alpha=0.7)
...
def plot_experiment3_comparison(results, save_path=None):
    ...
    bars = plt.bar(classifiers, accuracies, color=colors[:len(classifiers)], alpha=0.7)
...
def plot_experiment4_efficiency(results, save_path=None):
    ...
    bars = ax.bar(classifiers, build_times, color=colors[:len(classifiers)], alpha=0.7)
    ...
    ax.set_title('Classifier Build Time')
    ax.set_yscale('log')
    ax.grid(True, linestyle='--', alpha=0.3, axis='y')
    
    # 预测时间对比
    ax = axes[1]
    bars = ax.bar(classifiers, predict_times, color=colors[:len(classifiers)], alpha=0.7)
    ...
    ax.set_title('Classifier Predict Time')
    ax.grid(True, linestyle='--', alpha=0.3, axis='y')
...
# %% 运行所有实验
def run_all_ablation_experiments(output_dir="./ablation_results"):
    """运行所有消融实验"""
    import os
