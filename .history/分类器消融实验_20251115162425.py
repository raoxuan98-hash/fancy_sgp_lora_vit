...
def extract_features_and_labels(model, train_loader, test_loader, model_name, num_shots, cache_dir="cached_data/classifier_ablation"):
    """
    提取特征和标签，支持缓存功能
    ...
    参数:
        ...
        test_loader: 测试数据加载器
        model_name: 模型名称，用于缓存键
        num_shots: shot数量，用于缓存键
        cache_dir: 缓存目录
    """
    # 检查缓存
    ...
    if (os.path.exists(cache_path + "_train_features.pt") and
        ...
        os.path.exists(cache_path + "_test_labels.pt")):
...
train_features, train_labels, train_dataset_ids, test_features, test_labels, test_dataset_ids = extract_features_and_labels(vit, train_dataloader, test_dataloader, num_shots=num_shots)
