...
if 'Times New Roman' in available_fonts:
    ...
    serif_font = 'STIXGeneral'  # 默认使用DejaVu Serif，这是Linux系统常用字体
...
train_features, train_labels, train_dataset_ids, test_features, test_labels, test_dataset_ids = extract_features_and_labels(vit, train_dataloader, test_dataloader, model_name, num_shots=num_shots, iterations=iterations)
...
def grid_search_alpha1_alpha2(alpha1_min=0, alpha1_max=1.0, alpha2_min=0, alpha2_max=1.0,
                           alpha1_points=10, alpha2_points=10, return_dataset_wise=False):
    ...
    for i, alpha1 in enumerate(alpha1_values):
        for j, alpha2 in enumerate(alpha2_values):
            ...
            acc = evaluate_qda_classifier(alpha1, alpha2, alpha3_fixed, train_stats, test_features,
                                         test_labels, test_dataset_ids, device="cuda", return_dataset_wise=return_dataset_wise)
...
alpha1_values, alpha2_values, accuracy_matrix = grid_search_alpha1_alpha2(
    ...
    alpha1_points=11, alpha2_points=11, return_dataset_wise=False)
...
def plot_alpha1_alpha2_contour(alpha1_values, alpha2_values, accuracy_matrix, save_path=None):
    ...
    # 绘制等高线图
    contour = plt.contourf(alpha1_grid, alpha2_grid, accuracy_matrix.T, levels=50, cmap='viridis')
    ...
    # 添加等高线
    contour_lines = plt.contour(alpha1_grid, alpha2_grid, accuracy_matrix.T, levels=11, colors='black', alpha=0.4)
    ...
    plt.xlabel(r'$\alpha_1^{\rm rgda}$', fontsize=9)
    plt.ylabel(r'$\alpha_2^{\rm rgda}$', fontsize=9)
...
best_alpha1, best_alpha2, best_acc = plot_alpha1_alpha2_contour(
    ...
    save_path=f"实验结果保存/分类器消融实验/alpha1_alpha2_contour_{model_name}.png"
)
