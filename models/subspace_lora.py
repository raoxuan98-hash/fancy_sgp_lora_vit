...
class SubspaceLoRA(BaseLearner):
    def __init__(self, args: Dict[str, Any]) -> None:
        ...
        self.use_aux_for_kd: bool = args.get("use_aux_for_kd", True)
    ...
    def incremental_train(self, data_manager) -> None:
        ...
        # 创建 DataLoader，注意：使用 sampler �?shuffle 必须�?False
        self.train_loader_test_mode = DataLoader(
            train_set_test_mode,
            batch_size=self.batch_size,
            shuffle=False if sampler else True,  # �?sampler 时不�?shuffle
            sampler=sampler,
            num_workers=3,
            pin_memory=True,
        ...
        if self.aux_loader is None:
            ...
            self.drift_compensator.initialize_aux_loader(train_set)
    ...
    def process_batch(
        ...
        inputs: torch.Tensor,
        targets: torch.Tensor,
        optimizer: optim.Optimizer) -> Tuple[float, int, float]:
        """
        拼接原任务和参考数据集，一次 forward，减少计算。
        """
        ...
        if self.use_feature_kd:
            if self.use_aux_for_kd and self.aux_loader is not None:
                try:
                    aux_inputs, _ = next(self.aux_iter)
                except StopIteration:
                    # 到头重置迭代器
                    self.aux_iter = iter(self.aux_loader)
                    aux_inputs, _ = next(self.aux_iter)
                aux_inputs = aux_inputs.to(self._device)

        # === 拼接输入，一次 forward_features ===
        if aux_inputs is not None:
            concat_inputs = torch.cat([inputs, aux_inputs], dim=0)
        else:
            concat_inputs = inputs

        # teacher 网络特征
        with torch.no_grad():
            prev_feats_concat = self.prev_network.forward_features(concat_inputs)
        feats_concat = self.network.forward_features(concat_inputs)

        # 拆回两部分
        if aux_inputs is not None:
            b_main = inputs.size(0)
            b_aux = aux_inputs.size(0)
            prev_feats, prev_feats_aux = prev_feats_concat.split([b_main, b_aux], dim=0)
            feats, feats_aux = feats_concat.split([b_main, b_aux], dim=0)
        else:
            prev_feats = prev_feats_concat
            feats = feats_concat
            feats_aux = None
            prev_feats_aux = None

        if self.use_feature_kd and aux_inputs is not None:
            kd_feat = self.kd_loss_fn(prev_feats_aux, feats_aux)
            kd_norm = self.norm_loss(prev_feats_aux, feats_aux)
            kd_term = self.gamma_kd * kd_feat + self.gamma_norm * kd_norm
        elif self.use_feature_kd:
        ...
        if self.current_task_id >= 0 and self.network.vit.use_projection:
            ...
            if self.covariances is None:
                ...
                for k in self.covariances:
                    self.covariances[k] = 0.8 * self.covariances[k] + new_covs[k]
