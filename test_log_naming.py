...
def test_parameter_filtering():
    ...
    # Test SGP LoRA - should not have NSP parameters in params.json
    args['lora_type'] = 'sgp_lora'
    args['weight_temp'] = 2.0
    args['weight_kind'] = 'log1p'
    args['weight_p'] = 1.0
    args['nsp_eps'] = 0.05  # Should be filtered out
    args['nsp_weight'] = 0.02  # Should be filtered out
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Should have SGP parameters
        assert 'weight_temp' in params, f"weight_temp not found in params.json"
        assert 'weight_kind' in params, f"weight_kind not found in params.json"
        assert 'weight_p' in params, f"weight_p not found in params.json"
        
        # Should NOT have NSP parameters
        assert 'nsp_eps' not in params, f"nsp_eps found in params.json but should be filtered"
        assert 'nsp_weight' not in params, f"nsp_weight found in params.json but should be filtered"
