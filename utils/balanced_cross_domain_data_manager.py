...
class BalancedCrossDomainDataManagerCore(CrossDomainDataManagerCore):
    ...
    def _create_incremental_dataset(self, original_dataset: Dict, class_indices: List[int],
        ...
        # 创建标签映射：原始全局标签 -> 新的全局标签
        # 我们需要新的全局标签从 offset 开始，其中 offset 将在外部计算
        sorted_global_classes = sorted(global_class_indices)
        
        # 临时映射：原始全局标签 -> 从0开始的本地标签
        # 因为全局偏移会在 get_subset 中应用
        local_label_mapping = {orig_label: local_label for local_label, orig_label in enumerate(sorted_global_classes)}
        ...
        split_dataset = {
            ...
            'test_data': test_data,
            'train_targets': train_targets_local,  # 本地标签（0开始）
            'test_targets': test_targets_local,    # 本地标签（0开始）
            ...
            'original_global_offset': original_global_offset,  # 保存原始全局偏移
            'global_class_indices': global_class_indices  # 保存全局类别索引
        }
